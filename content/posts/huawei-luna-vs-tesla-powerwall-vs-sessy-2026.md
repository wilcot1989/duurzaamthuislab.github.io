---
title: Huawei Luna vs Tesla Powerwall vs Sessy 2026
date: 2026-06-04 09:00:00+01:00
lastmod: 2026-08-20 08:00:00+02:00
description: 'Huawei Luna 2000, Tesla Powerwall 3 en Sessy naast elkaar: prijs, capaciteit, slimheid en terugverdientijd vergeleken voor de Nederlandse markt in 2026.'
categories:
- thuisbatterijen
tags:
- Huawei Luna
- Tesla Powerwall
- Sessy
- thuisbatterij vergelijking
- thuisbatterij 2026
keywords:
- Huawei Luna vs Tesla Powerwall
- Sessy thuisbatterij
- beste thuisbatterij 2026
- thuisbatterij vergelijken
- Huawei Luna 2000 review
affiliate: false
author: Team DuurzaamThuisLab
author_bio: Team DuurzaamThuisLab schrijft datagedreven over zonnepanelen, thuisbatterijen en warmtepompen — op basis van specificaties, publieke data en narekenbare modelberekeningen.
featured_image: https://wsrv.nl/?url=images.unsplash.com/photo-1589276534126-adef63a95e05&w=1200&output=webp&q=70
faq:
- q: 'Welke thuisbatterij is het goedkoopst in 2026: Huawei Luna, Tesla Powerwall of Sessy?'
  a: De Huawei Luna 2000 begint het goedkoopst per kWh (€570-€760/kWh), gevolgd door de Sessy (ca. €750/kWh). De Tesla Powerwall 3 is het duurst in aanschaf (€8.500-€9.500) maar heeft wel de grootste vaste capaciteit van 13,5 kWh.
- q: Werkt de Sessy ook zonder zonnepanelen?
  a: Ja, de Sessy is specifiek ontworpen voor gebruik met een dynamisch energiecontract. Je laadt goedkoop op bij lage stroomtarieven (nacht, windpiek) en verbruikt wanneer het duurder is. Dit maakt de Sessy interessant voor huishoudens zonder zonnepanelen.
- q: Hoe lang duurt installatie van een Tesla Powerwall 3?
  a: Een Tesla Powerwall 3 installatie duurt gemiddeld 4-8 uur voor een gecertificeerd installateur. De wachttijd voor een installateur kan langer zijn — reken in drukke periodes op 4-8 weken na bestelling.
- q: Is de Huawei Luna compatibel met alle omvormers?
  a: Nee. De Huawei Luna 2000 werkt het beste in combinatie met een Huawei SUN2000 hybride omvormer. Gebruik je een andere omvormer, dan heb je een compatibiliteitschecker nodig of een extra interface-apparaat. Controleer dit altijd bij je installateur.
- q: Wat is het verschil tussen de Sessy en andere thuisbatterijen?
  a: 'De Sessy onderscheidt zich door zijn slimme handelsstrategie: de batterij kijkt naar de EPEX-spotprijzen en besluit automatisch wanneer te laden en ontladen voor maximale besparing. Dit maakt hem bij uitstek geschikt voor dynamische energiecontracten.'
- q: Welke thuisbatterij heeft de langste garantie?
  a: De Tesla Powerwall 3 en Huawei Luna 2000 bieden beide 10 jaar garantie met een minimale eindcapaciteit van 70%. De Sessy biedt 7 jaar garantie. Voor oudere systemen met meer laadcycli is de 10-jaarsgarantie een relevant voordeel.
- q: Kan ik een thuisbatterij ook gebruiken als back-up bij stroomuitval?
  a: Dat hangt af van het model. De Tesla Powerwall 3 heeft een ingebouwde back-up functie (off-grid capable). De Huawei Luna heeft dit ook met de juiste omvormer-configuratie. De Sessy biedt in de huidige versie geen standaard back-up bij stroomuitval.
- q: Wat is de terugverdientijd van een thuisbatterij in 2026?
  a: 'Zonder saldering (vanaf 2027) en met een dynamisch contract komen de meeste rekenmodellen uit op 8-12 jaar. De Sessy scoort in die modellen iets beter bij actief gebruik met dynamische tarieven, omdat de batterij ook op prijsverschillen handelt. De uitkomst hangt sterk af van de prijsspreiding op de spotmarkt.'
- q: 'Kan ik mijn Huawei Luna 2000 later uitbreiden?'
  a: 'Ja. De Luna 2000 is modulair — je kunt beginnen met 5 kWh en later uitbreiden naar 10 of 15 kWh door extra modules toe te voegen. Dat maakt de instapdrempel lager: begin klein en breid uit als de investering zich bewijst.'
- q: 'Werkt de Tesla Powerwall 3 met mijn bestaande SolarEdge omvormer?'
  a: 'Nee, niet naadloos. De Powerwall 3 heeft een ingebouwde DC-omvormer en is primair ontworpen als standalone systeem. Voor gebruik met een bestaande omvormer is een gateway nodig, en sommige functionaliteiten (zoals back-up) werken dan beperkt. Tesla raadt aan om bij een volledige omvormer-vervanging de Powerwall 3 te kiezen.'
- q: 'Hoe ver laad ik de Sessy op bij een dynamisch contract?'
  a: 'De Sessy laadt standaard tot 90-95% capaciteit op de goedkoopste uren. Je kunt in de app handmatig grenzen instellen, maar dat is zelden nodig — het algoritme optimaliseert automatisch op basis van de volgende dag''s prijscurve.'
- q: 'Kan ik meerdere Sessy''s naast elkaar plaatsen?'
  a: 'Ja. Je kunt meerdere Sessy-units in parallel plaatsen voor meer capaciteit. De units werken dan gecoördineerd via hetzelfde algoritme. Twee Sessy''s van elk 5 kWh geven 10 kWh capaciteit voor ca. €8.000-€8.500 — iets duurder per kWh dan de Huawei Luna 10 kWh, maar AC-gekoppeld en dus omvormer-onafhankelijk.'
products:
- name: Sessy thuisbatterij
  url: https://go.duurzaamthuislab.nl/sessy
  price: '4499'
- name: Tesla Powerwall 3
  url: https://go.duurzaamthuislab.nl/tesla-powerwall
  price: '9500'
- name: Huawei Luna 2000
  url: https://go.duurzaamthuislab.nl/huawei-luna
  price: '6000'
schema_type: Article
---
Drie jaar geleden had je amper keus als je een thuisbatterij wilde in Nederland. Nu is de markt vol, en zijn de drie namen die telkens terugkomen de Huawei Luna 2000, de Tesla Powerwall 3 en de Nederlandse nieuwkomer Sessy. Deze vergelijking is gebaseerd op de specificaties, handleidingen, garantievoorwaarden en gebruikersreviews van de drie systemen. Het doel: een eerlijk beeld van de verschillen, de kosten en de vraag welke past bij jouw situatie.

*Disclosure: de links naar Huawei, Sessy en Tesla in dit artikel zijn gewone verwijzingen — wij hebben met deze partijen geen affiliate- of commissierelatie.*

---


> **Kort antwoord:** Huawei Luna 2000, Tesla Powerwall 3 en Sessy naast elkaar: prijs, capaciteit, slimheid en terugverdientijd vergeleken voor de Nederlandse markt in 2026.
>
> De Huawei Luna 2000 begint het goedkoopst per kWh (€570-€760/kWh), gevolgd door de Sessy (ca. €750/kWh). De Tesla Powerwall 3 is het duurst in aanschaf (€8.500-€9.500) maar heeft wel de grootste vaste capaciteit van 13,5 kWh.

## Snelle vergelijking op hoofdpunten

| | **Huawei Luna 2000** | **Tesla Powerwall 3** | **Sessy** |
|---|---|---|---|
| **Capaciteit** | 5-15 kWh (modulair) | 13,5 kWh (vast) | 5 kWh |
| **Vermogen (continu)** | 2,5-5 kW | 5 kW | 3,68 kW |
| **Prijs (excl. installatie)** | €3.800-€8.500 | €8.500-€9.500 | €3.999-€4.499 |
| **Prijs per kWh** | €570-€760 | €630-€700 | ~€900 |
| **Garantie** | 10 jaar | 10 jaar | 7 jaar |
| **Back-up bij uitval** | Ja (met hybride omvormer) | Ja (ingebouwd) | Nee |
| **Zonder zonnepanelen** | Beperkt | Beperkt | Ja (dynamisch) |
| **Nederlandse installateur** | Breed netwerk | Beperkt netwerk | Eigen netwerk |
| **App-kwaliteit** | Goed | Uitstekend | Goed |

---

## Huawei Luna 2000: de werkhorse

De Huawei Luna 2000 is de bestverkochte thuisbatterij in Nederland. Dat is niet voor niets: de prijs-kwaliteitverhouding is sterk, het systeem is modulair opgebouwd (je begint met 5 kWh en voegt later modules toe), en Huawei heeft een breed netwerk van gecertificeerde installateurs.

### Hoe zo'n systeem in de praktijk draait

Een typische opstelling is een Luna 2000 van 10 kWh in combinatie met een Huawei SUN2000 hybride omvormer en circa 18 panelen. De FusionSolar-app is informatierijk, maar de interface is naar onze inschatting nogal technisch voor wie zich niet in de materie wil verdiepen.

Het seizoenspatroon is voorspelbaar: in de zomer wordt de batterij overdag op zonne-energie gevuld en 's avonds leeggetrokken, in de winter is de bijdrage veel kleiner omdat er simpelweg te weinig opbrengst is om 10 kWh te vullen. Reken er in de wintermaanden op dat de batterij slechts een deel van de avondpiek dekt.

### Sterke punten

- **Modulair:** Begin met 5 kWh, voeg later meer toe. Dat maakt de drempel lager.
- **Prijs per kWh:** Goedkoopste van de drie, zeker bij 10+ kWh configuraties.
- **Bewezen technologie:** Al jaren op de markt, grote hoeveelheid data over prestaties.
- **Breed installateurnetwerk:** Veel gecertificeerde partijen in Nederland.

### Zwakke punten

- **Gebonden aan Huawei-ecosysteem:** Werkt het best met Huawei-omvormer. Gebruik je een andere omvormer, dan zijn er complicaties.
- **Geen dynamische handelsstrategie:** De Luna laadt en ontlaadt op basis van zonne-energie en eenvoudige tijdschema's, maar koopt niet actief op spotmarkt-laagtepunten.
- **Chinese hardware:** Voor sommige mensen een politieke overweging.

### Geschikt voor

Huishoudens met zonnepanelen (bij voorkeur Huawei-omvormer) die een bewezen, betaalbaar systeem willen met uitbreidingsmogelijkheden.

---

## Tesla Powerwall 3: de premium keuze

De Tesla Powerwall 3 is het meest herkenbare product in dit rijtje. De derde generatie (uitgebracht in 2024) is technisch een grote stap vooruit: ingebouwde omvormer, hoog continu-vermogen (5 kW), en de beste back-up-functie van de drie.

### Wat maakt de Powerwall 3 anders?

Het grootste verschil met de vorige generaties: de Powerwall 3 heeft een ingebouwde omvormer. Dat betekent dat je bij nieuwbouw of vervanging van je omvormer geen apart apparaat meer nodig hebt. Voor bestaande installaties met een andere omvormer is dat ingewikkelder — je kunt de Powerwall 3 dan alleen als extra batterijopslag toevoegen via een gateway, maar je verliest wat functionaliteit.

De back-up bij stroomuitval is de beste in zijn klasse. Het systeem schakelt in milliseconden over, en je kunt zelfs bepalen welke groepen in huis beschermd worden.

### De app-ervaring

De Tesla-app is de meest gebruiksvriendelijke van de drie. Je ziet real-time hoeveel stroom er van de panelen komt, hoeveel de batterij opslaat en levert, en wat er van/naar het net gaat. Er zijn ook slimme schema's die je kunt instellen — maar een volledig dynamische strategie (à la Sessy) heeft Tesla niet.

### Sterke punten

- **Ingebouwde omvormer:** Eén apparaat voor panelen én opslag.
- **Beste back-up-functie:** Millisecondes-overschakeling, scheidbaar per groep.
- **Premium bouw en design:** Elegant, compact, weerbestendig.
- **Uitstekende app.**

### Zwakke punten

- **Prijs:** Duurste van de drie in aanschaf. Niet modulair.
- **Beperkt installateurnetwerk:** Fewer gecertificeerde Tesla-installateurs in Nederland.
- **Vaste capaciteit:** 13,5 kWh. Kun je niet kleiner nemen als je dat genoeg vindt.
- **Geen standaard dynamische handelsstrategie.**

### Geschikt voor

Huishoudens die een premium, all-in systeem willen met betrouwbare back-up, bij voorkeur bij nieuwbouw of vervanging van de omvormer.

---

## Sessy: de slimme Nederlander

Sessy is een Nederlandse startup (onderdeel van Homewizard-ecosysteem) en het meest bijzondere product in dit rijtje. Want de Sessy is niet primair gemaakt om zonne-energie op te slaan — het is een batterij die inkoopt op de spotmarkt.

### Hoe werkt de Sessy precies?

De Sessy is gekoppeld aan de EPEX-spotmarkt (de groothandelsmarkt voor elektriciteit). Iedere dag worden de uurprijzen voor de volgende dag gepubliceerd. De Sessy's algoritme bepaalt automatisch: wanneer laden (laagste prijs, vaak nacht of middag bij veel zonnestroom op het net) en wanneer ontladen (hoogste prijs, doorgaans avondspits).

Wat dat oplevert, is een kwestie van rekenen. Rekenvoorbeeld met de volgende aannames: een bruikbare capaciteit van 5 kWh, één volledige cyclus per dag, een retourrendement van 90% en een gemiddeld dagverschil tussen het goedkoopste en duurste uur van €0,10-€0,15 per kWh. Uitkomst van het model: circa €0,45-€0,70 voordeel per dag, ofwel grofweg €150-€230 per jaar. Bij een volatielere markt loopt dat op, bij een vlakke prijscurve valt het terug — de spread is de enige variabele die echt telt.

Bovenop de arbitrage werkt de Sessy ook gewoon als opslagbatterij voor zonne-energie als je die hebt.

### Is de Sessy ook geschikt zonder zonnepanelen?

Ja — en dat is uniek. Een apart artikel gaat hier uitgebreider op in ([batterij zonder zonnepanelen: heeft het zin?](/posts/batterij-na-2027-zonder-zonnepanelen-zin-2026)), maar de korte samenvatting: met een dynamisch contract en de Sessy kun je ook zonder panelen besparen. Niet zoveel als met panelen, maar wel degelijk merkbaar.

### Sterke punten

- **Slimste strategie van de drie:** Handelt actief op spotprijzen.
- **Werkt zonder zonnepanelen:** Uniek in dit segment.
- **Nederlands product:** Support, garantieafhandeling en installateurs zijn lokaal.
- **Lagere aanschafprijs dan Powerwall.**

### Zwakke punten

- **Kleinste capaciteit:** 5 kWh. Niet uitbreidbaar (althans, niet standaard).
- **Duurste per kWh:** Als je puur naar kWh-prijs kijkt, is de Sessy de duurste.
- **Geen back-up bij stroomuitval.**
- **Jonger merk:** Minder trackrecord dan Huawei en Tesla.
- **Rendement afhankelijk van prijsvariatie:** Weinig variatie (vlakke spotmarkt) = minder besparing.

### Geschikt voor

Huishoudens die een dynamisch energiecontract hebben of willen, en actief willen inspelen op prijsfluctuaties. Zeker interessant als aanvulling op bestaande zonnepanelen, maar ook als standalone.

---

## Gedetailleerde kostenvergelijking

### Aanschaf + installatie

| | **Huawei Luna 10 kWh** | **Tesla Powerwall 3** | **Sessy 5 kWh** |
|---|---|---|---|
| Systeemprijs | €6.500 | €9.000 | €4.200 |
| Installatie (schatting) | €1.200 | €1.500 | €700 |
| Totaal | **€7.700** | **€10.500** | **€4.900** |
| Prijs per kWh | **€770** | **€778** | **€980** |

### Jaarlijkse besparing (schatting)

| | **Huawei Luna 10 kWh** | **Tesla Powerwall 3** | **Sessy 5 kWh** |
|---|---|---|---|
| Zelf-consumptie stijging | +30-40% | +30-40% | +20-25% |
| Geschatte jaarlijkse besparing | €600-€900 | €700-€950 | €300-€600 (spotmarkt + ZP) |
| Terugverdientijd | 8-13 jaar | 11-15 jaar | 8-16 jaar |

*Let op: deze schattingen zijn indicatief en hangen sterk af van jouw verbruik, het aantal zonne-uren, de energieprijzen en het energiecontract.*

---

## Welke kies je?

Op basis van alle vergelijkingen hierboven valt de keuze als volgt uiteen:

**Kies Huawei Luna 2000 als:**
- Je al een Huawei-omvormer hebt (of er eentje gaat kopen)
- Je een groter systeem wilt (10 kWh of meer)
- Je prijs-kwaliteit boven premium-design stelt
- Je een bewezen, breed ondersteund systeem wil

**Kies Tesla Powerwall 3 als:**
- Je een nieuw systeem bouwt en een all-in oplossing wil
- Betrouwbare back-up bij stroomuitval voor jou essentieel is
- Je budget ruimer is en je het beste wil
- Je een nieuwbouwwoning of renovatie doet

**Kies Sessy als:**
- Je een dynamisch energiecontract hebt of wil
- Je ook zonder of met beperkte zonnepanelen wil besparen
- Je geïnteresseerd bent in actief inspelen op spotprijzen
- Je een kleinere investering wil doen om het systeem te proberen

---

## Een combinatie van twee systemen: zinvol of niet?

Wie al een Huawei Luna heeft, kan overwegen om een Sessy als aanvulling bij te plaatsen. De twee systemen zijn in principe complementair: de Luna slaat overdag zonne-energie op, de Sessy koopt in op de lage spotprijzen buiten de zonuren. Samen dekken ze een groter deel van de dag.

Is dat verstandig? Financieel is het een randgeval. De tweede batterij verdient zichzelf alleen terug als de spotmarkt genoeg variatie houdt, en dat is een aanname over meerdere jaren die niemand hard kan maken. De meerwaarde zit vooral in flexibiliteit en toekomstbestendigheid: naarmate dynamische contracten gangbaarder worden, wordt de waarde van een actief handelende batterij groter. Onze inschatting: voor de meeste huishoudens is één goed gedimensioneerd systeem verstandiger dan twee kleinere.

Bekijk de opties via de onderstaande links:

- [Sessy thuisbatterij](https://go.duurzaamthuislab.nl/sessy)
- [Tesla Powerwall 3](https://go.duurzaamthuislab.nl/tesla-powerwall)
- [Huawei Luna 2000](https://go.duurzaamthuislab.nl/huawei-luna)

---

## Installatie: wat je moet weten

De installatie van een thuisbatterij is niet hetzelfde als het plaatsen van een extra stopcontact. Het zijn complexe systemen met hoog-voltage DC-spanning en soms zelfs back-up functionaliteit die speciale elektrische aanpassingen vereist.

### Huawei Luna installatie

De Luna 2000 wordt altijd geïnstalleerd door een gecertificeerde Huawei-partner. De installatie omvat:
- Bevestiging van de batterijmodules op een muur (minimaal 30 cm vrij rondom voor ventilatie)
- Aansluiting op de Huawei SUN2000 hybride omvormer
- Configuratie in de FusionSolar-app
- Melding aan de netbeheerder (vereist voor systemen boven 1 kWp)

Installatieduur: 4-6 uur voor een 10 kWh systeem. Wachttijd op een installateur: 2-8 weken afhankelijk van regio en seizoen.

**Waar let je op?**
- De Luna moet in een vorstvrije ruimte hangen (min. 0°C)
- De muurbevestiging vereist een stevige wand (steen of beton, geen gipsplaat)
- Elke module weegt 30-40 kg — reken op een twee-mans installatieteam

### Tesla Powerwall 3 installatie

De Powerwall 3 installeert Tesla uitsluitend via gecertificeerde installateurpartners — en dat netwerk is in Nederland beperkter dan bij Huawei. Wachttijden van 4-12 weken zijn realistisch.

De Powerwall 3 heeft een ingebouwde omvormer. Bij een bestaande installatie met een andere omvormer kan de Powerwall 3 worden toegevoegd als aanvullende opslag via een gateway, maar dat is technisch complexer en beperkt sommige functies.

**Bijzonderheid:** De Powerwall 3 werkt optimaal in combinatie met het Tesla Powerwall Gateway 2 — een separaat apparaat dat het systeem regelt en back-up functionaliteit mogelijk maakt. Dat kost €400-€700 extra.

### Sessy installatie

Sessy werkt met een eigen installateurnetwerk. De installatie is eenvoudiger dan bij de andere twee: de Sessy werkt als een AC-gekoppelde batterij (geen hybride omvormer nodig) en kan dus naast elke bestaande omvormer worden geplaatst.

Installatieduur: 2-4 uur. Wachttijden zijn doorgaans korter dan bij Huawei of Tesla.

**Voordeel:** Je hebt geen nieuwe omvormer nodig. Dat scheelt €1.500-€3.000 als je al een werkende omvormer hebt.

---

## Monitoring en apps vergeleken

De kwaliteit van de monitoring-app maakt een groot verschil in de dagelijkse beleving van je thuisbatterij.

### FusionSolar (Huawei)

De FusionSolar-app is informatierijk maar technisch van insteek. Je ziet per minuut wat de panelen produceren, wat de batterij opslaat en levert, en wat er van het net wordt afgenomen. De interface is functioneel maar minder intuïtief voor niet-techneuten.

Pluspunten:
- Gedetailleerde historische data (per uur, dag, maand, jaar)
- Alarm-meldingen bij afwijkende prestaties
- Mogelijkheid tot remote configuratie

Minpunten:
- Chinese serverinfrastructuur (voor wie dat een punt is)
- Interface kan overweldigend zijn voor beginners

### Tesla-app

De Tesla-app is het meest gepolijste gebruikerservaring van de drie. De visualisatie van energie-stromen (panelen → batterij → huis → net) is intuïtief en aantrekkelijk, ook voor wie zich niet in de techniek wil verdiepen — dat is in gebruikersreviews een terugkerend compliment.

Pluspunten:
- Meest gebruiksvriendelijke interface
- Real-time waarschuwingen bij storingen
- Integratie met Tesla-auto (V2H in toekomst)
- Time-based control: stel in wanneer de batterij laadt (bijv. op goedkope uren)

Minpunten:
- Minder gedetailleerde technische data dan FusionSolar
- Afhankelijk van Tesla-servers

### Sessy-app

De Sessy-app richt zich op het zichtbaar maken van besparingen door dynamisch laden. Je ziet de EPEX-spotprijzen, het laadschema van de batterij, en een overzicht van de dagelijkse en maandelijkse besparing.

Pluspunten:
- Spotprijzen direct zichtbaar in de app
- Automatisch laadschema inzichtelijk
- Nederlandse ondersteuning en interface

Minpunten:
- Minder gedetailleerde energie-flow visualisatie
- Relatief nieuw systeem — bugs en verbeteringen worden regelmatig uitgerold

---

## Garantie en levensduur: eerlijk vergelijken

### Huawei Luna 2000

- **Garantie:** 10 jaar
- **Minimale eindcapaciteit na 10 jaar:** 70%
- **Levensduur batterijcellen:** 6.000 laadcycli (bij dagelijks gebruik: 16-17 jaar)
- **Garantieafhandeling:** Via gecertificeerde Huawei-installateur, reactietijd 2-5 werkdagen

### Tesla Powerwall 3

- **Garantie:** 10 jaar
- **Minimale eindcapaciteit na 10 jaar:** 70% (bij max. 37,8 MWh totale doorvoer)
- **Levensduur:** 10.000+ laadcycli (LFP-batterijchemie)
- **Garantieafhandeling:** Via Tesla, doorgaans snelle respons

### Sessy

- **Garantie:** 7 jaar
- **Minimale eindcapaciteit:** 80% na 4.000 cycli
- **Levensduur:** Circa 10-12 jaar bij normaal gebruik
- **Garantieafhandeling:** Via Sessy/Homewizard, Nederlands support

De kortere garantietermijn van de Sessy is een reëel nadeel bij een investering van €4.000+. Bedenk dat na 7 jaar een groot defect voor eigen rekening is, terwijl de Huawei en Tesla nog 3 jaar garantie geven.

---

## Milieuoverwegingen

Een thuisbatterij is een verduurzamingsinvestering, maar heeft ook een milieu-footprint.

**Productie:** Lithium-ijzerfosfaat batterijen (LFP, gebruikt door de meeste moderne thuisbatterijen) hebben een gunstiger milieuprofiel dan oudere lithium-cobalt-oxide chemie. Er wordt geen kobalt gebruikt, en de productie-emissies zijn lager.

**Levensduur:** Een batterij die 15 jaar meegaat, verdient haar CO₂-voetafdruk in productie ruimschoots terug via de vermeden uitstoot van grijze stroom.

**Recycling:** In Europa is recycling van batterijen geregeld via de Wet milieubeheer. Zorg bij vervanging of afdanken dat de batterij bij een erkend recyclingbedrijf terechtkomt. Alle drie de merken hebben recyclingprogramma's.

---

## Ons eindoordeel

Op basis van de specificaties, garantievoorwaarden en prijsstelling komen de drie batterijen er zo uit:

**Voor de gemiddelde huiseigenaar met zonnepanelen en een bestaande niet-Huawei omvormer:** Sessy of een goedkoper AC-gekoppeld systeem.

**Voor de huiseigenaar met Huawei-omvormer:** Huawei Luna zonder twijfel.

**Voor nieuwbouw of complete systeemvervanging met maximale ambitie:** Tesla Powerwall 3.

Er is geen universele winnaar. De beste thuisbatterij is de batterij die past bij jouw omvormer, jouw budget, jouw gebruik en jouw risicotolerantie. Laat je niet verleiden door marketingtaal — reken je eigen situatie door.

[Sessy thuisbatterij](https://go.duurzaamthuislab.nl/sessy) | [Tesla Powerwall 3](https://go.duurzaamthuislab.nl/tesla-powerwall) | [Huawei Luna 2000](https://go.duurzaamthuislab.nl/huawei-luna)

---

## Subsidies voor thuisbatterijen in 2026

Een veelgestelde vraag: is er subsidie voor thuisbatterijen? Het korte antwoord is nee, niet vanuit het Rijk. Wat er wel en niet geldt, zetten we hieronder op een rij.

### De ISDE dekt geen thuisbatterijen

De Investeringssubsidie Duurzame Energie (ISDE) voor woningeigenaren dekt volgens RVO uitsluitend isolatie, ventilatie (in combinatie met isolatie), (hybride) warmtepompen, zonneboilers, een aansluiting op een warmtenet en elektrisch koken. Thuisbatterijen staan er niet op — en zonnepanelen en laadpalen ook niet. Voor alle drie de batterijen in deze vergelijking (Huawei Luna 2000, Tesla Powerwall 3 en Sessy) geldt dus hetzelfde: geen rijkssubsidie, ook niet in combinatie met zonnepanelen of een warmtepomp.

**Wat er wél is:**
- Gemeentelijke en provinciale regelingen voor thuisopslag. Die bestaan, maar verschillen per gemeente en zijn vaak snel uitgeput — check de regeling van je eigen gemeente en vraag aan vóór de installatie.
- Het effect van de salderingsafbouw: hoe minder een teruggeleverde kWh opbrengt, hoe waardevoller opslag voor eigen gebruik wordt. Dat is geen subsidie, maar het beweegt de businesscase wel.

Reken een batterij dus altijd door op de volle prijs. Wie een offerte krijgt waarin een "ISDE-subsidie op de batterij" is ingeboekt, moet daar doorvragen.

### Btw: 21% op de batterij, 0% op de panelen

Voor zonnepanelen voor woningen geldt het nultarief: 0% btw op levering en installatie van de panelen plus wat direct nodig is om ze aan te brengen en te laten functioneren — kabels, PV-verdeler, montagemateriaal, optimizers, omvormers en aanpassingen aan meterkast en dak.

De Belastingdienst stelt expliciet dat de levering en installatie van een accupakket en thuisbatterij daar buiten valt. Op de batterij betaal je dus 21% btw, ook als je hem gelijktijdig met de panelen koopt en op dezelfde factuur laat zetten. Staat er in een offerte 0% btw over de accu, dan is dat een fout die je laat corrigeren.

Eén uitzondering ligt bij ondernemerschap: wie als btw-ondernemer stroom teruglevert, kan de btw op de installatie in specifieke gevallen terugvragen. De Belastingdienst stelt daar voorwaarden aan, waaronder een energiemanagementsysteem en een dynamisch contract. Bekijk de actuele voorwaarden op belastingdienst.nl of leg het voor aan je boekhouder — het is geen automatisch voordeel.

---

## Veiligheid en brandrisico

Thuisbatterijen bevatten grote hoeveelheden energie. Veiligheid is een terechte zorg.

**LiFePO4 vs. NMC-chemie**
Alle drie de modellen (Huawei Luna, Tesla Powerwall 3, Sessy) gebruiken lithium-ijzerfosfaat (LiFePO4) als batterijchemie. Dit is de veiligste lithium-batterijchemie — het risico op thermische runaway (brand) is significant lager dan bij oudere NMC- of NCA-chemie.

**Plaatsingsregels**
- Niet in een slaapkamer of kinderkamer plaatsen
- Minimaal 50 cm vrij rondom de batterij voor ventilatie
- Vorstvrije ruimte (min. 0°C)
- Niet in een ruimte met brandbare materialen of gasinstallaties
- Bij voorkeur in een garage of technische ruimte

**Brandverzekering**
Controleer bij je verzekeraar of een thuisbatterij gedekt is onder je inboedel- of opstalverzekering. De meeste verzekeraars dekken thuisbatterijen standaard, maar bij grote systemen (>10 kWh) kan een aanvullende melding vereist zijn.

---

## Toekomstbestendigheid: welke batterij gaat het langst mee?

De technologie van thuisbatterijen ontwikkelt snel. Welke van de drie is het meest toekomstbestendig?

**Huawei Luna:** Gebonden aan het Huawei-ecosysteem. Als Huawei stopt met de Nederlandse markt (politiek risico), kan dit gevolgen hebben voor software-updates en garantieondersteuning. Technisch robuust systeem.

**Tesla Powerwall 3:** Tesla heeft een sterke marktpositie en investeert zwaar in thuisenergieopslag (Powerwall, Megapack). Het risico dat Tesla stopt met de consumentenmarkt is laag. De 10-jaarsgarantie en 10.000+ cycli geven de langste levensduurgarantie van de drie.

**Sessy:** Jong merk, maar deel van het Homewizard-ecosysteem dat groeit. Het risico is groter dan bij Huawei of Tesla — een startup kan verdwijnen of worden overgenomen. De 7-jaarsgarantie is korter. Dat weegt mee bij een beslissing voor de lange termijn.

Als toekomstbestendigheid jouw hoogste prioriteit is: Tesla Powerwall 3. Als prijs-kwaliteit en ecosysteem-fit jouw prioriteit is: Huawei Luna (met Huawei-omvormer). Als flexibiliteit en slimheid voor een dynamisch contract je prioriteit is: Sessy.


<a href="https://go.duurzaamthuislab.nl/sessy" class="cta cta-affiliate" rel="nofollow noopener" target="_blank">Bekijk Sessy</a>

## Gerelateerde artikelen

- [Beste Thuisbatterij 2026: Top 5 voor Nederlandse Huishoudens](/posts/beste-thuisbatterij-nederland-2026/)
- [Beste 10 kWh thuisbatterij 2026: vergelijking 7 topmerken](/posts/thuisbatterij-10-kwh-vergelijking-2026/)
- [Sessy review 2026: eerlijke test van de Nederlandse](/posts/sessy-review-thuisbatterij-nederland/)
- [Sessy Radar 2026: software-update + dynamisch](/posts/sessy-software-update-2026-radar/)
- [Thuisbatterij Zonder Zonnepanelen: Heeft Het Zin in 2027?](/posts/batterij-na-2027-zonder-zonnepanelen-zin-2026/)

---

**Externe bron:** [RVO — ISDE voor woningeigenaren](https://www.rvo.nl/subsidies-financiering/isde/woningeigenaren) — het officiële overzicht van welke maatregelen de ISDE wel en niet dekt (thuisbatterijen, zonnepanelen en laadpalen vallen er niet onder).
