---
title: 'Beste Powerstation Blackout & Noodgeval 2026'
date: 2026-10-12 08:00:00+02:00
lastmod: '2026-08-21 08:00:00+02:00'
draft: false
description: "Powerstation voor stroomuitval: hoeveel kWh je nodig hebt en welke leverbare modellen dat halen, met geverifieerde prijzen van 21 augustus 2026."
categories:
- powerstations
tags:
- powerstation
- blackout
- noodstroom
- LiFePO4
- EcoFlow
- Jackery
- Bluetti
keywords:
- beste powerstation blackout
- powerstation noodgeval
- noodstroom thuis 2026
- powerstation stroomuitval
- beste powerstation 2026
affiliate: true
author: Team DuurzaamThuisLab
author_bio: Team DuurzaamThuisLab schrijft datagedreven over zonnepanelen, thuisbatterijen en warmtepompen — op basis van specificaties, publieke data en narekenbare modelberekeningen.
featured_image: https://wsrv.nl/?url=images.unsplash.com/photo-1542621334-a254cf47733d&w=1200&output=webp&q=70
faq:
- q: 'Hoeveel kWh heb ik nodig voor een powerstation bij stroomuitval?'
  a: 'Voor een gemiddeld huishouden met koelkast, cv-pomp, router en verlichting reken je op 275–325 W continu. Een batterij van 2 kWh houdt dat pakket volgens onze modelberekening ruim 5,5 uur vol, 4 kWh ruim 11 uur. Uitbreidbare systemen tot 12 kWh komen op circa 33 uur. Reken daarbij met ongeveer 90 procent bruikbare capaciteit door omvormerverlies.'
- q: 'Is een powerstation veilig om binnen te gebruiken?'
  a: 'Ja. Een powerstation produceert geen uitlaatgassen en mag gewoon binnen staan. Dat is het grote voordeel ten opzichte van een benzineaggregaat, dat je nooit binnen mag draaien vanwege koolmonoxide.'
- q: 'Kan ik mijn CV-ketel aansluiten op een powerstation?'
  a: 'Ja, zolang je powerstation minstens 800W piekvermogen aankan en een zuivere sinusgolf levert. CV-pompen en -ketels zijn gevoelig voor vervormde spanning. Alle modellen in dit artikel leveren volgens de fabrikantopgave een zuivere sinusgolf.'
- q: 'Hoe lang duurt opladen van een leeg powerstation?'
  a: 'Dat verschilt per model en volgt uit de maximale netingang. De EcoFlow Delta 2 Max neemt volgens de specificatie tot 2.300 W op: 2.048 Wh ÷ 2.300 W is rekenkundig 0,89 uur, dus krap een uur. De Delta Pro 3 neemt tot 2.900 W op: 4.096 Wh ÷ 2.900 W is 1,41 uur, dus krap anderhalf uur. Jackery geeft voor de Explorer 2000 v2 een volle lading in 1,33 uur op. Dit zijn rekenkundige minima — in de praktijk zakt het laadvermogen boven ongeveer 80 procent terug, dus de laatste procenten duren langer.'
- q: 'Werkt een powerstation samen met zonnepanelen tijdens een blackout?'
  a: 'Ja. Je sluit losse panelen aan op de MC4-ingangen van het powerstation; het net hoeft daarvoor niet beschikbaar te zijn. Reken niet met het paneelvermogen maar met de realistische opbrengst: met 400 Wp aan panelen haal je bij felle zon eerder 250–320 W, wat neerkomt op ongeveer 1 kWh per 3 tot 4 uur.'
- q: 'Waarom geeft mijn thuisbatterij (Sessy, Marstek) geen noodstroom?'
  a: 'Thuisbatterijen zijn standaard gekoppeld aan het net en schakelen bij stroomuitval ook uit — voor de veiligheid van monteurs op het net. Noodstroom werkt alleen met een aparte backup-voorziening: bij Sessy is dat een noodstroom-basisinstallatie van €1.200, en alleen in combinatie met de Plus. Een powerstation heeft die functie al ingebouwd.'
- q: 'LiFePO4 of NMC — welke chemie is beter voor noodstroom?'
  a: 'LiFePO4 (lithiumijzerfosfaat) is veiliger bij volledige ontlading, haalt in de fabrieksopgaven 3.000 tot 4.000 cycli en degradeert langzamer. NMC heeft een hogere energiedichtheid, dus een lichter apparaat per kWh, maar in de opgaven duidelijk minder cycli. Voor thuisgebruik en noodstroom kies je bij voorkeur LiFePO4.'
- q: 'Kan ik een powerstation ook dagelijks gebruiken voor energie-arbitrage?'
  a: 'Technisch wel: bij een dynamisch contract laad je op goedkope uren en gebruik je de energie wanneer stroom duur is. Modellen met een UPS-modus maken dat praktisch mogelijk. Maar de cycli tellen op, en voor dagelijkse arbitrage is een vaste thuisbatterij economisch interessanter.'
products:
- name: EcoFlow Delta 2 Max
  url: https://go.duurzaamthuislab.nl/ecoflow
  price: '949'
- name: Jackery Explorer 2000 v2
  url: https://go.duurzaamthuislab.nl/jackery
  price: '899'
- name: EcoFlow Delta Pro 3
  url: https://go.duurzaamthuislab.nl/ecoflow
  price: '3299'
schema_type: Article
last_updated: '2026-08-21'
---

*Volledige vergelijking van alle typen: [beste draagbare powerstation 2026](/posts/beste-draagbare-powerstation-2026/) — deze pagina is de keuzehulp voor noodstroom bij stroomuitval.*

*Disclosure: wij hebben affiliate-partnerships met EcoFlow, Jackery, Bluetti en Anker Solix via AWIN. Wij vergelijken op basis van specificaties, handleidingen en publieke data.*

---
Bij een storing in het net komen dezelfde twee vragen op: hoe lang houd ik het vol zonder stroom, en wat moet ik daarvoor kopen? Het eerste is rekenwerk, het tweede volgt daaruit.

Dit artikel geeft dat rekenwerk en vergelijkt daarna de modellen die in Nederland leverbaar zijn. De prijzen hieronder komen van de Nederlandse webshops van de fabrikanten, opgehaald op 21 augustus 2026. Waar wij een prijs niet hebben kunnen verifiëren, noemen wij geen bedrag. Wij hebben deze apparaten niet zelf getest; alle draaitijden zijn modelberekeningen met de aannames erbij.

> **Kort antwoord:** wie één nacht wil overbruggen zit goed met een station van circa 2 kWh: de **EcoFlow Delta 2 Max** (€949, van €1.199) of de **Jackery Explorer 2000 v2** (€899) — beide prijzen van 21 augustus 2026. Wil je een cv-ketel én zwaardere apparatuur voeden, of een etmaal en meer overbruggen, dan is de **EcoFlow Delta Pro 3** (4.096 Wh, 4.000 W, €3.299) het model dat dat volgens de specificatie haalt.

---

## Hoeveel kWh heb je écht nodig?

Voor de modellen aan bod komen, eerst het rekenmodel. Stel: je wilt bij een blackout het **essentiële huishouden** draaiende houden:

| Apparaat | Vermogen |
|---|---|
| Koelkast (cyclisch, gemiddeld) | 100 W |
| CV-pomp + ketelbesturing | 80 W |
| Router + modem | 15 W |
| LED-verlichting (3 ruimtes) | 20 W |
| Laptop + telefoonladers | 60 W |
| **Totaal gemiddeld continu** | **275–325 W** |

**Aannames van het model:** een constante afname van 325 W (de bovenkant van de bandbreedte) en circa 90 procent bruikbare capaciteit, omdat de ingebouwde omvormer verlies heeft bij het omzetten naar 230 V. Via de 12V-uitgangen is dat verlies kleiner en haal je meer uren uit dezelfde batterij.

| Nominale capaciteit | Backup-duur bij 325 W |
|---|---|
| 1 kWh | ~2 uur 45 min |
| 2 kWh | ~5 uur 30 min |
| 4,1 kWh | ~11 uur 20 min |
| 6 kWh | ~16 uur 30 min |
| 12 kWh | ~33 uur |

Voor de meeste Nederlandse stroomuitvallen — een paar uur tot een halve dag — is 2 tot 4 kWh dus genoeg. Pas bij langdurige winterstormen of regionale netproblemen wordt 6 kWh of meer relevant.

**Klein huishouden** (appartement, 1–2 personen, geen cv-pomp): 1–2 kWh volstaat voor een nacht.  
**Middelgroot huishouden** (rijtjeshuis, 2–4 personen, cv-ketel): 2–4 kWh voor een halve tot hele dag.  
**Groot huishouden of thuiskantoor**: 6–12 kWh voor een volledige dag plus nacht.

---

## De modellen op een rij

### 1. EcoFlow Delta 2 Max — de logische standaardkeuze

**Capaciteit:** 2.048 Wh | **AC-vermogen:** 2.400 W continu, 4.800 W piek | **Chemie:** LiFePO4 | **Cycli:** >3.000 tot 80% | **Netladen:** tot 2.300 W | **Solar-input:** 500 W per poort, 1.000 W met twee poorten | **Gewicht:** 23 kg | **Garantie:** 5 jaar | **Prijs:** €949 (van €1.199), nl.ecoflow.com op 21 augustus 2026

Dit is het model waar de meeste huishoudens aan toe hebben. 2.400 W continu is genoeg voor de cv-ketel, de koelkast, verlichting, netwerkapparatuur en een waterkoker — niet alles tegelijk, maar wel achter elkaar. Een inductiekookplaat op vol vermogen valt erbuiten.

De netingang van 2.300 W is voor noodstroom belangrijker dan hij lijkt: krijg je een stormwaarschuwing, dan is 2.048 Wh ÷ 2.300 W rekenkundig 0,89 uur — krap een uur, en in de praktijk iets langer omdat het laadvermogen richting vol terugzakt.

**Modelberekening draaitijd** (2.048 Wh, 90 procent bruikbaar = circa 1.840 Wh):

- Essentieel pakket (325 W): **circa 5 uur 40 min**
- Alleen koelkast (100 W gemiddeld): **circa 18 uur**
- Cv-ketel plus router en licht (115 W): **circa 16 uur**

**Nadelen:** 23 kg is geen tilgewicht voor iedereen, en de ventilator is bij zware belasting hoorbaar. Wie zowel de cv als inductie wil kunnen voeden, komt vermogen tekort.

**Voor wie:** het overgrote deel van de huishoudens die een storing van enkele uren tot een nacht willen overbruggen.

<a href="https://go.duurzaamthuislab.nl/ecoflow?ref=/posts/beste-powerstation-blackout-noodgeval-2026/" class="cta cta-affiliate" target="_blank" rel="noopener nofollow sponsored">Bekijk de EcoFlow Delta 2 Max</a>

Zie ook onze uitgebreide [EcoFlow Delta 2 Max review](/posts/ecoflow-delta-2-max-review-2026/) en de [EcoFlow vs Jackery vergelijking](/posts/ecoflow-vs-jackery-2026/) als je twijfelt tussen de merken.

---

### 2. Jackery Explorer 2000 v2 — vergelijkbare capaciteit, tien kilo lichter

**Capaciteit:** 2.042 Wh | **AC-vermogen:** 2.200 W nominaal | **Chemie:** LiFePO4 | **Cycli:** 4.000 | **Netladen:** volle lading in 1,33 uur (fabrikantopgave) | **Gewicht:** 17,5 kg | **Garantie:** 3 + 2 jaar | **Prijs:** €899, nl.jackery.com op 21 augustus 2026

Let hier op de modelnaam. De Explorer **2000 Plus** — het model dat in veel vergelijkingen nog als aanbeveling staat, inclusief eerdere versies van dit artikel — stond op 21 augustus 2026 **niet in het Nederlandse assortiment** van Jackery. Het Nederlandse overzicht bestond uit de Explorer 500 v2 (€329), 1000 v2 (€519), 2000 v2 (€899) en 3000 v2 (€1.599). Wie de Plus zoekt is dus aangewezen op resterende dealervoorraad of de internationale winkel, met de garantievraag die daarbij hoort. Het volledige verschil tussen beide staat in [Explorer 2000 v2 vs 2000 Plus](/posts/jackery-explorer-2000-v2-vs-plus-2026/).

De v2 heeft praktisch dezelfde bruikbare energie als de Delta 2 Max en is met 17,5 kg ruim vijf kilo lichter. Het continu vermogen ligt met 2.200 W iets lager, en de opgegeven 4.000 cycli is gelijk aan de hoogste opgave in dit overzicht — de Delta Pro 3 geeft er ook 4.000 op.

**Modelberekening draaitijd** (2.042 Wh, 90 procent bruikbaar = circa 1.840 Wh):

- Essentieel pakket (325 W): **circa 5 uur 40 min**
- Alleen koelkast (100 W gemiddeld): **circa 18 uur**

**Nadelen:** de 2.200 W is de harde grens — een zware kookplaat of heteluchtoven haalt hij niet. Op de Nederlandse productpagina stond voor de v2 geen uitbreidingsaccu, dus meer energie betekent hier een groter toestel in plaats van een extra module. En let op de prijs: €899 was op onze peildatum de webshopprijs. Een doorgestreepte adviesprijs ernaast hebben wij niet kunnen verifiëren — op de Nederlandse overzichtspagina is €1.599 de prijs van de grótere Explorer 3000 v2 — dus reken met €899 en controleer het bedrag op het moment dat je koopt.

**Voor wie:** huishoudens zonder inductie die één nacht willen overbruggen en het station ook voor [camping](/posts/beste-powerstation-camping-2026/) of [vanlife](/posts/beste-powerstation-vanlife-2026/) willen gebruiken — daar telt die tien kilo minder wél.

<a href="https://go.duurzaamthuislab.nl/jackery?ref=/posts/beste-powerstation-blackout-noodgeval-2026/" class="cta cta-affiliate" target="_blank" rel="noopener nofollow sponsored">Bekijk de Jackery Explorer 2000 v2</a>

---

### 3. EcoFlow Delta Pro 3 — voor cv plus inductie, en voor een etmaal of meer

**Capaciteit:** 4.096 Wh (uitbreidbaar tot 12 kWh) | **AC-vermogen:** 4.000 W continu, 8.000 W met X-Boost | **Chemie:** LiFePO4 | **Cycli:** 4.000 tot 80% | **Netladen:** tot 2.900 W | **Solar-input:** tot 2.600 W over twee poorten | **UPS-overschakeltijd:** 10 ms | **Gewicht:** 51,5 kg | **Garantie:** 5 jaar | **Prijs:** €3.299, nl.ecoflow.com op 21 augustus 2026

Dit is de klasse waarin de combinatie cv-ketel plus inductiekookplaat wel past: 4.000 W continu dekt een piekbelasting die de 2 kWh-modellen afkappen. Twee dingen maken dit model specifiek geschikt als vaste noodstroomvoorziening.

Het eerste is de **UPS-overschakeltijd van 10 ms** volgens de specificatie. Desktopcomputers, NAS-systemen en gevoelige elektronica lopen bij zo'n omschakeling door zonder te herstarten. Werk je thuis met een desktop en twee schermen, of draai je een NAS met belangrijke data, dan is dat het argument.

Het tweede is de **uitbreidbaarheid tot 12 kWh** met extra accu's. Daarmee schuift dit model in het gebied waar je een volledige dag en nacht overbrugt.

**Modelberekening draaitijd** (4.096 Wh, 90 procent bruikbaar = circa 3.690 Wh):

- Essentieel pakket (325 W): **circa 11 uur 20 min**
- Alleen koelkast (100 W gemiddeld): **circa 37 uur**
- Inductie op halve stand plus essentieel (1.200 W): **circa 3 uur**
- Uitgebreid naar 12 kWh bij 325 W: **circa 33 uur**

**Nadelen:** 51,5 kg is verplaatsbaar, niet draagbaar — je zet dit toestel één keer neer. En de prijs is het viervoudige van een 2 kWh-station voor het dubbele aan energie; je betaalt vooral voor vermogen, uitbreidbaarheid en de snelle omschakeling.

**Voor wie:** huishoudens met cv én inductie, thuiskantoren met servers of NAS, en iedereen die een etmaal of langer wil overbruggen.

<a href="https://go.duurzaamthuislab.nl/ecoflow?ref=/posts/beste-powerstation-blackout-noodgeval-2026/" class="cta cta-affiliate" target="_blank" rel="noopener nofollow sponsored">Bekijk de EcoFlow Delta Pro 3</a>

---

### 4. Bluetti AC500 + B300K — het meest schaalbare systeem

**Capaciteit:** 2.764,8 Wh per B300K-module, volgens de fabrikantopgave op te bouwen met zes modules (6 × 2.764,8 Wh = circa 16,6 kWh) | **AC-vermogen:** 5.000 W continu | **Chemie:** LiFePO4 | **UPS-modus:** 20 ms volgens Bluetti | **Prijs:** €1.200 voor het pakket "AC500 + B300K Home Battery Backup", afgeprijsd van €3.999; een losse extra B300K-module kost €1.399 (bluettipower.eu, gecontroleerd 21-8-2026)

De AC500 is geen powerstation in de gebruikelijke zin: het is een omvormer/lader zonder ingebouwde batterij. Je koppelt er altijd één of meer batterijmodules aan; op de Europese webshop is dat sinds deze prijsronde de **B300K** (2.764,8 Wh) en niet meer de B300S. Dat maakt het systeem het meest schaalbare in dit overzicht, en het is de reden dat het voor grotere woningen in beeld komt.

**Modelberekening draaitijd** (1 × B300K, 2.764,8 Wh nominaal, 90 procent bruikbaar = circa 2.488 Wh):

- Essentieel pakket (325 W): **circa 7 uur 40 min**
- Desktopkantoor plus essentieel (600 W): **circa 4 uur**
- Met twee modules (5.529,6 Wh, circa 4.977 Wh bruikbaar) bij 325 W: **circa 15 uur 20 min**

**Prijs per kWh capaciteit.** Het instappakket komt uit op €1.200 ÷ 2,7648 kWh = circa €434 per kWh, en dat is met 5.000 W continu erbij het gunstigste vermogen-voor-je-geld in dit overzicht: de EcoFlow Delta 2 Max staat op €949 ÷ 2,048 kWh = circa €463 per kWh bij 2.400 W en de Delta Pro 3 op €3.299 ÷ 4,096 kWh = circa €805 per kWh. Uitbreiden is wél duurder dan instappen: een tweede module kost €1.399, ofwel circa €506 per kWh.

**Nadelen:** je hebt altijd minimaal één batterijmodule nodig, want de AC500 doet zelf niets. De opbouw is complexer dan bij een all-in-one station. De €1.200 is een sterk afgeprijsd actiebedrag (van €3.999), dus controleer het op het moment dat je koopt, en let erop dat de garantietermijn per onderdeel kan verschillen.

**Voor wie:** grotere huishoudens die stap voor stap naar 6 tot 18 kWh willen groeien.

<a href="https://go.duurzaamthuislab.nl/bluetti-eu?ref=/posts/beste-powerstation-blackout-noodgeval-2026/" class="cta cta-affiliate" target="_blank" rel="noopener nofollow sponsored">Bekijk de actuele Bluetti-prijzen</a>

Zie ook onze [Bluetti AC200Max review](/posts/bluetti-ac200max-review-2026/) voor het kleinere, niet-modulaire model — let op: de AC200MAX stond op 21 augustus 2026 niet meer als toestel in de Bluetti EU-shop; de opvolger daar is de AC200PL (2.304 Wh, 2.400 W, €1.499).

---

### 5. Anker Solix F2000 (PowerHouse 767) — snel bijladen

**Capaciteit:** 2.048 Wh | **AC-vermogen:** 2.300 W continu | **Chemie:** LiFePO4 | **Cycli:** 3.000+ volgens Anker | **Netladen:** circa 1 uur volgens Anker | **Prijs:** €1.499 (ankersolix.com/nl, gecontroleerd 21-8-2026)

De F2000 is in de opgaven de snelste netlader in dit overzicht: circa een uur voor een volle lading. Dat is precies de eigenschap die telt als je bij een stormwaarschuwing nog wilt bijvullen, of als je het station ook dagelijks laadt en ontlaadt. Anker geeft daarbij op GaN-techniek in de laadelektronica te gebruiken, wat de warmteontwikkeling bij snelladen beperkt.

**Modelberekening draaitijd** (2.048 Wh nominaal, 90 procent bruikbaar = circa 1.843 Wh):

- Essentieel pakket (325 W): **circa 5 uur 40 min**
- Alleen koelkast (100 W gemiddeld): **circa 18 uur**
- Volledig bijladen via 300 W realistische zonopbrengst: **circa 7 uur zon**

**Nadelen:** de capaciteit is niet uitbreidbaar, dus wie later meer wil, koopt een tweede toestel. Inductie op vol vermogen lukt met 2.300 W net niet. En de prijs weegt hier zwaar: €1.499 ÷ 2,048 kWh = circa €732 per kWh, tegen circa €463 per kWh voor de even grote EcoFlow Delta 2 Max (€949) en circa €440 voor de Jackery Explorer 2000 v2 (€899 ÷ 2,042 kWh). Je betaalt bij de F2000 dus ruim €500 meer voor dezelfde 2 kWh; het snelle netladen en de garantie moeten dat verschil waard zijn.

**Voor wie:** huishoudens die snel willen kunnen bijladen en geen schaalbaarheid nodig hebben.

<a href="https://go.duurzaamthuislab.nl/anker-solix?ref=/posts/beste-powerstation-blackout-noodgeval-2026/" class="cta cta-affiliate" target="_blank" rel="noopener nofollow sponsored">Bekijk de actuele Anker Solix-prijzen</a>

Zie ook onze [Anker Solix C1000 review](/posts/anker-solix-c1000-review-2026/) voor het kleinere model.

---

### Waarom de Goal Zero Yeti 6000X hier niet als aanbeveling staat

De Yeti 6000X komt in noodstroomlijstjes voor omdat hij met 6 kWh de meeste capaciteit in één kast heeft. Twee dingen houden ons tegen om hem aan te bevelen: de NMC-chemie levert in de fabrieksopgaven aanzienlijk minder laadcycli dan de LiFePO4-modellen hierboven, en het toestel weegt zoveel dat verplaatsen zonder wielen geen optie is. Voor een unit die je een paar keer per jaar gebruikt is dat cyclusverschil minder erg dan voor dagelijks gebruik, maar bij gelijke capaciteit kiezen wij LiFePO4.

Wij hebben voor dit model geen Nederlandse prijs kunnen verifiëren en noemen er daarom geen. De achtergrond staat in onze [Goal Zero Yeti 6000X review](/posts/goal-zero-yeti-6000x-review-2026/). Wij hebben geen affiliate-relatie met Goal Zero en ontvangen voor dit model geen vergoeding.

---

## Powerstation vs noodaggregaat (benzine)

Een veelgestelde vraag, en het antwoord is minder eenzijdig dan vaak wordt gedacht.

Voor een powerstation is de kostprijs per doorgezette kWh te berekenen uit de aanschafprijs en de opgegeven cycluslevensduur. **Modelberekening met de geverifieerde prijzen van 21 augustus 2026 en de cyclusopgaven van de fabrikanten:**

| Model | Prijs | Cycli (opgave) | Afschrijving per kWh |
|---|---|---|---|
| Jackery Explorer 2000 v2 | €899 | 4.000 × 2,042 kWh | circa €0,11 |
| EcoFlow Delta 2 Max | €949 | 3.000 × 2,048 kWh | circa €0,15 |
| EcoFlow Delta Pro 3 | €3.299 | 4.000 × 4,096 kWh | circa €0,20 |
| Anker Solix F2000 | €1.499 | 3.000 × 2,048 kWh | circa €0,24 |

Daar komt de laadstroom bij. Met onze rekenconstante van **€0,26 per kWh all-in** kom je zo op ongeveer €0,37 tot €0,50 per kWh die je uit het station haalt. Dat is een stuk gunstiger dan de €0,60 tot €1,00 die eerdere versies van dit artikel noemden — die berekening ging uit van 500 cycli, en dat aantal hoort bij NMC-chemie, niet bij de LiFePO4-modellen hier.

Een aggregaat is bij aanschaf goedkoper per watt vermogen en levert energie zolang je brandstof hebt. Wat een kWh uit een aggregaat kost, hangt volledig af van de benzineprijs en het verbruik bij jouw belasting; wij noemen daarvoor geen bedrag dat wij niet hebben nagerekend op een geverifieerde brandstofprijs.

Wat wél vaststaat, zijn de praktische bezwaren die in een Nederlandse woonwijk zwaar wegen:

- **Koolmonoxide**: een aggregaat mag **nooit** binnen draaien, ook niet in de garage of een halfopen schuur. CO is kleurloos, geurloos en levensgevaarlijk.
- **Lawaai**: een benzineaggregaat is in de hele straat hoorbaar. In stedelijk gebied levert dat problemen op met de VvE of de gemeente.
- **Brandstofopslag**: benzine gaat in bewaring achteruit, en jerrycans in de schuur zijn een brandrisico.
- **Onderhoud**: carburateur, bougie en olie vragen periodiek werk, juist bij een apparaat dat je zelden gebruikt.

Voor een boerderij in een dunbevolkt gebied waar een lange uitval realistisch is, kan een aggregaat zinvol zijn. Voor een rijtjeshuis in een woonwijk is een powerstation de praktische keuze.

---

## Powerstation vs thuisbatterij (Sessy, Marstek)

Ook een veelgestelde vraag, met een helder antwoord: **een standaard thuisbatterij geeft je geen noodstroom bij een blackout**.

Thuisbatterijen zoals de [Sessy of Marstek](/posts/sessy-vs-marstek-thuisbatterij-2026/) zijn bedoeld voor zelfverbruik, dynamisch laden en zonnepaneeloptimalisatie. Ze zijn op het net aangesloten en **schakelen bij stroomuitval automatisch uit** — dat is vereist voor de veiligheid van monteurs aan het net.

Noodstroom werkt alleen als er een aparte backup-voorziening is die de woning tijdens de storing van het net scheidt. Bij Sessy kost die noodstroom-basisinstallatie €1.200 en is hij alleen beschikbaar in combinatie met de Plus. Dat bedrag komt dus bovenop de batterij zelf.

Een powerstation heeft die functie standaard ingebouwd. Je sluit de apparaten aan die je wilt beschermen, en bij stroomverlies schakelt het toestel over.

Zie voor meer context onze [thuisbatterij prijzen vergelijking](/posts/thuisbatterij-prijzen-vergelijking-2026/) en het [dynamisch contract met batterij rekenmodel](/posts/dynamisch-contract-met-batterij-rekenmodel-2026/).

---

## Solar bijladen tijdens een blackout

Duurt de storing langer dan je batterijcapaciteit, dan is zon de enige manier om bij te laden zonder net.

**Aannames van dit model:** een paneel van 400 Wp levert onder ideale omstandigheden 400 W. Realistisch in Nederland is 250–320 W bij felle zomerzon, 100–180 W bij bewolking en 30–60 W in de winter.

- Bij **300 W realistisch zonnig** laad je **1 kWh per 3 uur 20 minuten**.
- Een lege EcoFlow Delta 2 Max (2.048 Wh) vul je bij 300 W in ongeveer **7 uur zon** — bijna een hele zomerse dag.
- Een lege Jackery Explorer 2000 v2 (2.042 Wh) vraagt bij 300 W praktisch dezelfde tijd.
- Een lege Delta Pro 3 (4.096 Wh) vraagt bij 300 W ongeveer **14 uur zon**: dat lukt dus niet op één dag, tenzij je meer paneelvermogen aansluit. De solar-ingang van 2.600 W maakt dat mogelijk.

Kijk daarbij naar de maximale solar-input van het station: bij de Delta 2 Max is dat 500 W per poort en 1.000 W over twee poorten, bij de Delta Pro 3 tot 2.600 W. Meer panelen aansluiten dan de ingang aankan levert niets extra op.

De meeste powerstations hebben MC4-ingangen voor directe aansluiting van panelen; een omvormer is daarvoor niet nodig. Een combinatie van een **[zonnepaneel plus powerstation](/posts/beste-zonnepaneel-powerstation-combinatie-2026/)** maakt je bij een langdurige zomerse storing vrijwel zelfvoorzienend.

Let op: bij een bewolkte winterstorm kun je met 400 Wp rekenen op 50 tot 100 W. Dan is bijladen weinig effectief en is capaciteit in kWh je enige buffer.

---

## Aankoop-tips

**Btw en fiscaal**

Powerstations vallen onder het normale btw-tarief van 21 procent. Er is geen verlaagd tarief van toepassing, ook niet als je het toestel samen met zonnepanelen koopt. Voor zonnepanelen zelf en de direct noodzakelijke onderdelen geldt sinds 2023 het 0 procent-tarief; een powerstation of thuisbatterij valt daar niet onder.

**Tweedehands kopen**

LiFePO4-accu's degraderen langzaam maar gestaag, en bij een tweedehands toestel weet je niet hoeveel cycli erop zitten. Vraag om een schermfoto van de cycle count uit de app van EcoFlow, Jackery of Bluetti. Op een LiFePO4-station met een opgave van 3.000 tot 4.000 cycli is een paar honderd cycli verwaarloosbaar; bij NMC-chemie weegt hetzelfde aantal veel zwaarder.

**Garantie**

Voor de modellen hierboven staat in de opgaven: EcoFlow 5 jaar op de Delta 2 Max en de Delta Pro 3, Jackery 3 plus 2 jaar op de Explorer 2000 v2. Voor Bluetti en Anker Solix hebben wij de actuele termijn niet kunnen verifiëren — controleer die op de productpagina. Koop bij voorkeur bij een Nederlands of Europees verkooppunt: dan geldt de EU-consumentenbescherming en is retourneren eenvoudiger.

**Vermogen controleren**

Controleer of je apparaten een **zuivere sinusgolf** vereisen. Cv-ketels, medische apparatuur en sommige frequentiegeregelde motoren accepteren geen aangepaste sinusgolf. Alle modellen in dit artikel leveren volgens de fabrikantopgave een zuivere sinusgolf; bij de goedkoopste powerstations op de markt is dat niet altijd zo.

---

## Vergelijkingstabel

Prijzen zijn de webshopprijzen van de fabrikanten, opgehaald op 21 augustus 2026: nl.ecoflow.com, nl.jackery.com, bluettipower.eu en ankersolix.com/nl. Waar wij een gegeven niet hebben kunnen verifiëren, staat dat er.

| Model | Capaciteit | AC-vermogen | Chemie | Cycli (opgave) | Garantie | Prijs (21-8-2026) |
|---|---|---|---|---|---|---|
| EcoFlow Delta 2 Max | 2.048 Wh | 2.400 W (4.800 W piek) | LiFePO4 | >3.000 | 5 jaar | €949 (van €1.199) |
| Jackery Explorer 2000 v2 | 2.042 Wh | 2.200 W | LiFePO4 | 4.000 | 3 + 2 jaar | €899 |
| EcoFlow Delta Pro 3 | 4.096 Wh (tot 12 kWh) | 4.000 W (8.000 W X-Boost) | LiFePO4 | 4.000 | 5 jaar | €3.299 |
| Bluetti AC500 + B300K | 2,76 kWh per module, tot circa 16,6 kWh | 5.000 W | LiFePO4 | opgave fabrikant | niet geverifieerd | €1.200 (van €3.999) voor AC500 + 1× B300K; losse B300K €1.399 |
| Anker Solix F2000 | 2.048 Wh | 2.300 W | LiFePO4 | 3.000+ | niet geverifieerd | €1.499 |

---

## Welk model past bij jou?

**Rijtjeshuis met cv-ketel en inductie:** EcoFlow Delta Pro 3 (4.000 W continu, 4.096 Wh, €3.299) of het Bluetti AC500-systeem (5.000 W continu, €1.200 met één B300K-module van 2.764,8 Wh). Op vermogen kan de Bluetti het ruimer, en voor een derde van het geld; de Delta Pro 3 heeft er wel 1,3 kWh meer capaciteit en de snellere omschakeling (10 tegen 20 ms) tegenover staan.

**Appartement of kleine woning, geen inductie:** EcoFlow Delta 2 Max of Jackery Explorer 2000 v2. Vergelijkbare capaciteit; de Jackery is lichter en heeft de hoogste cyclusopgave, de EcoFlow heeft meer continu vermogen en een hogere solar-input.

**Thuiskantoor met servers of NAS:** EcoFlow Delta Pro 3 — de opgegeven overschakeltijd van 10 ms is de laagste hier.

**Maximale backup-duur:** Delta Pro 3 met extra accu's (tot 12 kWh), of het Bluetti AC500-systeem met meerdere B300K-modules (zes modules = circa 16,6 kWh, elke extra module €1.399).

**Onder €1.000:** Jackery Explorer 2000 v2 (€899) of EcoFlow Delta 2 Max (€949) — beide actieprijzen van 21 augustus 2026, dus controleer het bedrag bij aankoop.

Zie ook ons overzicht van [draagbare powerstations](/posts/beste-draagbare-powerstation-2026/) als je ook een lichtere unit voor onderweg zoekt.

---

Wil je het powerstation vast aansluiten op je meterkast of op een aparte groep, dan is dat werk voor een gecertificeerde installateur; achtergrondinformatie over energie in de gebouwde omgeving staat bij de [Rijksdienst voor Ondernemend Nederland](https://www.rvo.nl/onderwerpen/energie-en-klimaat-gebouwde-omgeving). Gebruik je het toestel als losse unit met stekkers, dan gelden er geen bijzondere regels — dat is gewoon consumentengebruik.
