---
title: 'Vergunning Warmtepomp Aanvragen 2026: Wanneer Verplicht?'
date: 2026-07-14 08:00:00+02:00
lastmod: 2026-07-14 08:00:00+02:00
description: 'Heb ik vergunning nodig voor mijn warmtepomp? Geluidsnorm 40 dB op buurtgrens, vergunningvrij of WABO-procedure. Stappenplan en kosten voor 2026 in Nederland.'
draft: false
categories:
- warmtepompen
tags:
- vergunning
- WABO
- omgevingswet
- geluidsnorm
- warmtepomp buiten
keywords:
- vergunning warmtepomp
- warmtepomp omgevingsvergunning
- warmtepomp geluidsnorm
- warmtepomp 40 db
- warmtepomp buurtgrens
- warmtepomp vergunningvrij
- warmtepomp aanvraag gemeente
author: Mark Bakker
author_bio: Energieadviseur met een eigen verduurzaamd huis. Test zonnepanelen, thuisbatterijen en warmtepompen in de praktijk.
featured_image: https://wsrv.nl/?url=images.unsplash.com/photo-1581094271901-8022df4466f9&w=1200&output=webp&q=70
schema_type: Article
affiliate: true
faq:
- q: 'Heb ik vergunning nodig voor een warmtepomp?'
  a: 'In de meeste gevallen niet, mits je voldoet aan de geluidsnorm van 40 dB(A) op de erfgrens. Bij overschrijding of plaatsing in beschermd stadsgezicht/monument: wel vergunning nodig.'
- q: 'Wat is de geluidsnorm voor warmtepompen?'
  a: 'Sinds 1 april 2021: 40 dB(A) op de erfgrens van de buren tussen 23:00 en 07:00 (nachtnorm). Overdag mag het meer zijn maar minimaal 50 dB(A) gemiddeld. Veel moderne pompen halen dit zonder problemen.'
- q: 'Hoe meet ik geluid van mijn warmtepomp?'
  a: 'De fabrikant vermeldt het geluidsvermogen (in dB(A) Lw). Je kunt een schatting doen met formules of een geluidsmeter app, maar voor een officiële melding heb je een akoestisch rapport van een specialist nodig (€350-€600).'
- q: 'Wat als ik in een Vinex-wijk woon?'
  a: 'Geluidseisen zijn extra strikt door dichte bebouwing. Plaats de buitenunit zo ver mogelijk van de erfgrens en gebruik geluidsdempende ombouw. Bij twijfel: vraag akoestisch rapport vooraf, het voorkomt klachten.'
- q: 'Kost een vergunning veel?'
  a: 'Indien nodig: €150-€650 leges bij gemeente, plus €350-€700 voor akoestisch rapport. Behandeltijd 8-12 weken. Veel goedkoper en sneller is voldoen aan vergunningvrije eisen.'
- q: 'Wat als de buren klagen?'
  a: 'Eerst zelf praten. Bij voortdurende klacht: gemeente komt meten. Bij overschrijding norm: aanpassingsplicht (geluidskap, verplaatsing). Voorkom dit met juiste plaatsing en geluidsbewuste keuze.'
products:
- name: Quatt warmtepomp
  url: https://www.quatt.nl/
  price: '8500'
- name: ATAG warmtepomp
  url: https://www.atag-verwarming.nl/
  price: '11000'
- name: Daikin Altherma 3
  url: https://www.daikin.nl/
  price: '13500'
---

Een buurman van mij in Zwolle plaatste eind 2024 een warmtepomp zonder na te denken over geluid. Twee maanden later stonden zijn buren bij de gemeente. Resultaat: gemeente kwam meten, bevond 44 dB(A) op erfgrens (4 dB boven norm), en buurman moest binnen 6 maanden geluidskap (€1.200) plaatsen of pomp verplaatsen (€2.500). Het had hem allemaal kunnen besparen met 2 uur planning vooraf.

Hieronder mijn complete gids voor vergunningen en geluidsregels rond warmtepompen in 2026. Wat is verplicht, wat is wijs, en hoe voorkom je problemen met buren of gemeente.

*Dit artikel bevat affiliate links. Bij aankoop via mijn links ontvang ik mogelijk een commissie, zonder extra kosten voor jou.*

## Wat verandert er met de Omgevingswet?

Sinds 1 januari 2024 geldt de Omgevingswet. Voor warmtepompen in praktijk betekent dit:
- Gelijke landelijke geluidsnorm (40 dB(A) erfgrens nacht)
- Gemeenten kunnen lokale aanvullingen maken
- Procedure via DSO (Digitaal Stelsel Omgevingswet) — vergelijkbaar met oude WABO

Geluidsnorm is sinds 1 april 2021 al landelijk geldig (Activiteitenbesluit milieubeheer aangepast). De Omgevingswet bouwt hierop voort.

## Wanneer is een vergunning nodig?

### Vergunningvrij — meestal het geval

Geen vergunning nodig als je voldoet aan ALLE van:
1. **Geluidsnorm**: max 40 dB(A) op erfgrens tussen 23:00-07:00
2. **Bouwbesluit**: pomp niet groter dan toegestane afmetingen
3. **Bestemmingsplan**: niet in beschermd stadsgezicht of bij monument
4. **Welstandsregels**: niet zichtbaar vanaf openbare weg (sommige gemeenten)
5. **Hoogte**: maximaal aan de gevel of op platte daken
6. **Afstand**: minimaal 1 meter van erfgrens (bij sommige gemeenten)

In 80%+ van de gevallen is plaatsing dus vergunningvrij — maar het hangt af van je gemeente.

### Vergunning nodig

Wel WABO-aanvraag indienen (via Omgevingsloket) als:
- **Beschermd stadsgezicht** (binnenstad Amsterdam, Utrecht, Delft, etc.)
- **Monument**: bij plaatsing aan/op monument of in directe omgeving
- **Welstandsregels**: gemeente eist beoordeling
- **Geluidsoverschrijding**: pomp >40 dB(A) op erfgrens
- **Constructieve aanpassing**: bij grote wijzigingen aan dak

## De 40 dB(A)-regel uitgelegd

Sinds 1 april 2021 geldt: een warmtepomp mag op de erfgrens van de buren maximaal 40 dB(A) produceren tussen 23:00 en 07:00. Dit is een nachtnorm — overdag mag het meer (50 dB(A) gemiddeld).

Wat is 40 dB(A)?
- 30 dB: fluisteren
- 40 dB: rustige bibliotheek
- 50 dB: rustig kantoor
- 60 dB: normale conversatie
- 70 dB: stofzuiger

Voor moderne warmtepompen is 40 dB(A) op erfgrens vrijwel altijd haalbaar bij goede plaatsing. Probleem ontstaat bij:
- Krappe achtertuinen (< 5 meter naar erfgrens)
- Kleine pompen tegen de gevel
- Oudere modellen met luide ventilator

## Berekening geluid op erfgrens

Eenvoudige formule:
- Geluidsvermogen pomp Lw (dB(A)) - 8 - 20×log10(afstand)
- Voorbeeld: pomp van 56 dB(A) Lw op 5m afstand → 56 - 8 - 14 = 34 dB(A) — voldoet
- Voorbeeld: pomp van 64 dB(A) Lw op 3m afstand → 64 - 8 - 9,5 = 46,5 dB(A) — overschrijdt!

Vermenigvuldig afstand met 2 → -6 dB minder. Dus van 3m naar 6m scheelt 6 dB.

Meer details bij [warmtepomp geluid buren norm](/posts/warmtepomp-geluid-buren-2026-norm/).

## Akoestisch rapport: wanneer wijs?

Een akoestisch rapport (€350-€600 door gespecialiseerd bureau) is wijs in deze gevallen:

1. Vinex-wijk of dichte bebouwing
2. Pomp >50 dB(A) Lw
3. Beoogde plaatsing dichter dan 4m van erfgrens
4. Beschermde geluidssituatie (lange weg verbeteren bestaande situatie)
5. Buren met eerdere klachten over geluid

Het rapport beschrijft:
- Geluidsmodel met 3D-software
- Berekening op verschillende ontvangpunten (erfgrenzen)
- Aanbevelingen voor mitigatie

## Mijn aanbevolen plaatsings-strategie

Voor de gemiddelde rijtjeshuis met 6m diepe achtertuin:

**Beste**: Pomp tegen de eigen achtergevel, gericht naar eigen tuin.
**Acceptabel**: Pomp in achtertuin minimum 3m van erfgrens.
**Slecht**: Pomp aan zijgevel direct tegen erfgrens.

Hoogte: liefst op 1-2 meter boven grond — geluid plant zich anders voort.

Geluidsdempende maatregelen die ik vaak adviseer:
- **Geluidskap rondom pomp**: -3 tot -8 dB
- **Akoestische lamellen schutting**: -3 tot -5 dB
- **Anti-trillingsmatten onder unit**: -2 dB
- **Slimme nachtmodus** (lagere ventilator-snelheid 's nachts): -3 dB

Lees ook [beste warmtepomp rijtjeshuis](/posts/beste-warmtepomp-rijtjeshuis-2026/).

## Vergunningsprocedure: stap voor stap

Als je toch een vergunning nodig hebt:

### Stap 1: Vooronderzoek

Check via [omgevingsloket.nl](https://www.omgevingsloket.nl) of je adres in beschermd gebied ligt. Bel je gemeente bij twijfel.

### Stap 2: Akoestisch rapport (indien nodig)

Vraag rapport bij specialist (zoek "akoestisch onderzoek warmtepomp" + je provincie). Doorlooptijd: 2-3 weken.

### Stap 3: Aanvraag indienen

Via DSO/Omgevingsloket online. Bijlagen:
- Situatieschets met afmetingen
- Foto's bestaande situatie
- Akoestisch rapport
- Specificaties warmtepomp
- Eventueel welstandsadvies

Leges: €150-€650 afhankelijk gemeente.

### Stap 4: Beoordeling (8-12 weken)

Gemeente toetst. Mogelijk vragen om aanvulling. Soms welstandscommissie inschakelen.

### Stap 5: Beschikking

Vergunning verleend (meestal) of geweigerd (zeldzaam). Bij weigering: bezwaar mogelijk.

### Stap 6: Bezwaartermijn

6 weken waarin buren bezwaar kunnen maken. Daarna onherroepelijk.

### Stap 7: Plaatsing

Pas dan installeren! Niet eerder beginnen — bouw zonder vergunning kan resulteren in handhavings­actie.

## Mythes ontkracht

**Mythe 1: "Boven 50 kW altijd vergunning"**
Niet waar. Geluidsnorm bepaalt, niet vermogen.

**Mythe 2: "Vergunningvrij = geen klachten mogelijk"**
Niet waar. Buren kunnen altijd klagen; dan komt gemeente meten.

**Mythe 3: "ATAG/Daikin/Quatt is altijd vergunningvrij"**
Niet automatisch. Hangt af van plaatsing en omgeving.

**Mythe 4: "Nachtmodus oplossing voor alles"**
Helpt, maar moet permanent zijn — niet alleen tijdens metingen door gemeente.

**Mythe 5: "Buren in eigen huis kunnen niet klagen"**
Wel: bij appartementencomplex of dubbelhuis is "erfgrens" de scheidende muur. Geluidsnorm geldt ook bij verbonden woningen.

## Specifieke situaties

### Vrijstaande woning ruim perceel

Vrijwel altijd vergunningvrij. Plaats pomp niet onder slaapkamerraam buren — dat is een sociale fout, geen wettelijk probleem.

### Rijtjeshuis Vinex-wijk

Krappere situatie. Plan vooraf. Akoestisch rapport bij twijfel.

### Appartement complex

Vergunningvrijheid afhankelijk VVE-statuten en gemeente. Vrijwel altijd VVE-toestemming nodig (zie [VVE zonnepanelen stappenplan](/posts/vve-zonnepanelen-stappenplan-2026/) voor vergelijkbare procedure).

### Monumenten

Vrijwel altijd vergunning. Plaatsing vaak alleen achterzijde of niet zichtbaar. Eventueel binnenopstelling met luchtkanalen.

Lees ook [zonnepanelen monumentaal pand](/posts/zonnepanelen-monumentaal-pand-2026/) voor vergelijkbare regelgeving.

### Beschermd stadsgezicht

Welstandsadvies vrijwel altijd nodig. Soms specifieke installatie-eisen (kleur, hoogte, type).

## Wat als de buren al hebben geklaagd?

Stappen:
1. **Praat met buren**: vaak op te lossen met kleine aanpassing (geluidskap, ander tijdregime)
2. **Doe zelf metingen**: app op je telefoon ('Sound Meter' van Abc Apps) — niet officieel maar indicatief
3. **Vraag offerte voor mitigatie**: kap, scherm, herplaatsing
4. **Bij blijvende klacht: bemiddeling buurtbemiddelaars**: vaak gratis via gemeente

Pas wanneer dit niet lukt: gemeente betrekt en officiële metingen. Dan ben je in een formele procedure.

## Kosten overzicht

| Situatie | Kosten | Tijd |
|----------|--------|------|
| Vergunningvrij standaard | €0 | 0 weken |
| Vergunningvrij + zelf meten | €0 | 1 week |
| Akoestisch rapport vooraf | €450 | 3 weken |
| Wel vergunning + rapport | €1.000-€1.300 | 12 weken |
| Achteraf mitigatie | €800-€2.500 | weken-maanden |

Investeren in goede planning (€450 rapport vooraf) is bijna altijd beter dan €1.500 mitigatie achteraf.

## Tips voor goede plaatsing

1. **Zo ver mogelijk van erfgrens**: vergroot afstand verlaagt geluid sterk
2. **Tegen eigen gevel, gericht naar eigen tuin**: gevel reflecteert geluid van buren weg
3. **Op stevige montageplaat (geen losliggende tegels)**: voorkomt trillingen
4. **Met fabrikants-vlucht spoiler**: vermindert ventilator-geluid
5. **Niet pal onder slaapkamerraam buren** (eigen of buren)
6. **Configureer nachtmodus** in installatie-instellingen
7. **Onderhoud**: vuile ventilator maakt meer geluid

## Welke pompen zijn stilst?

Bij gelijk vermogen (8 kW thermisch):
- **Daikin Altherma 3**: 53 dB(A) Lw — top
- **Mitsubishi Ecodan**: 54 dB(A) Lw
- **ATAG Energion**: 55 dB(A) Lw
- **Quatt Quattro**: 58 dB(A) Lw
- **Itho Daalderop Heatpump**: 60 dB(A) Lw

Lees [beste warmtepomp merk](/posts/beste-warmtepomp-merk-2026/) voor mijn complete vergelijking.

## Conclusie: planning bespaart geld

Vergunningen voor warmtepompen zijn meestal niet nodig, mits je vooraf goed plant. Mijn checklist:
1. Check geluidsspecificaties pomp (zoek <56 dB(A) Lw)
2. Plan plaatsing minimaal 3m van erfgrens
3. Bij twijfel: akoestisch rapport vóór installatie (€450)
4. Investeer in geluidsdempende ombouw indien nodig
5. Praat met buren VOORDAT je installeert — voorkomt 90% van klachten

Een paar uur extra planning bespaart vaak duizenden euro's en ruzie met de buurt. Bij twijfel altijd specialist raadplegen.

Lees ook: [warmtepomp installateur kiezen](/posts/beste-warmtepompinstallateur-kiezen-2026/), [beste warmtepomp 2026](/posts/beste-warmtepomp-merk-2026/), [warmtepomp installatie kosten](/posts/warmtepomp-installatie-kosten-2026/), [warmtepomp geluid buren norm](/posts/warmtepomp-geluid-buren-2026-norm/) en [hybride vs volledige warmtepomp](/posts/hybride-vs-volledige-warmtepomp-2026/).
