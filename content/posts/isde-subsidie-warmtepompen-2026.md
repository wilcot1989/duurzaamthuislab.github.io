---
title: 'ISDE Subsidie 2026: Complete Gids voor Warmtepompen &'
date: 2026-04-06 10:00:00+01:00
lastmod: 2026-08-20 08:00:00+02:00
description: 'Alles over de ISDE subsidie in 2026: bedragen voor warmtepompen en isolatie, hoe aanvragen, voorwaarden en combinatie met andere subsidies.'
categories:
- warmtepompen
tags:
- ISDE
- subsidie
- warmtepomp
- isolatie
- verduurzaming
keywords:
- ISDE subsidie
- subsidie warmtepomp 2026
- ISDE aanvragen
- warmtepomp subsidie Nederland
affiliate: false
author: Team DuurzaamThuisLab
author_bio: Team DuurzaamThuisLab schrijft datagedreven over zonnepanelen, thuisbatterijen en warmtepompen — op basis van specificaties, publieke data en narekenbare modelberekeningen.
featured_image: https://wsrv.nl/?url=plus.unsplash.com/premium_photo-1715620329644-36b9f5b9fca3&w=1200&output=webp&q=70
faq:
- q: Hoeveel ISDE-subsidie krijg je voor een warmtepomp?
  a: De ISDE subsidie voor een warmtepomp varieert van €3.000 tot €5.800 afhankelijk van het type. Een hybride warmtepomp levert circa €3.000 op, een lucht-water warmtepomp €3.000-€3.500, en een bodem-water warmtepomp €4.500-€5.800. Het exacte bedrag hangt af van het vermogen en de SCOP-waarde.
- q: Hoe vraag je ISDE-subsidie aan?
  a: 'Je vraagt ISDE subsidie aan via mijn.rvo.nl na installatie van de maatregel. Je hebt nodig: eHerkenning of DigiD, de factuur van de installateur, bewijs van betaling en de productspecificaties. De aanvraag moet binnen 12 maanden na installatie worden ingediend.'
- q: Is ISDE-subsidie te combineren met andere subsidies?
  a: Ja, de ISDE is te combineren met gemeentelijke subsidies en met een lening van het Nationaal Warmtefonds tegen lage rente. Een aparte isolatiesubsidie bovenop de ISDE bestaat niet meer. De oude SEEH kon tot en met 31 december 2022 worden aangevraagd en isolatie voor eigenaar-bewoners loopt sindsdien via de ISDE zelf, met hogere bedragen bij twee of meer maatregelen. VvE's vallen onder de SVVE.
- q: Hoe lang duurt het voordat de ISDE-subsidie wordt uitbetaald?
  a: Na het indienen van een complete aanvraag duurt het gemiddeld 8-13 weken voordat de subsidie wordt uitbetaald. In drukke perioden kan dit oplopen tot 16 weken. Zorg dat alle documenten compleet zijn om vertragingen te voorkomen.
- q: Geldt ISDE subsidie ook voor huurwoningen?
  a: De ISDE subsidie voor warmtepompen is beschikbaar voor eigenaar-bewoners van bestaande woningen. Huurders kunnen geen ISDE aanvragen, maar verhuurders en woningcorporaties kunnen dit wel doen voor hun bezit.
- q: Is er een budget limiet voor ISDE in 2026?
  a: Ja, het totale ISDE budget voor 2026 bedraagt circa €328 miljoen. In voorgaande jaren was het budget ruim voldoende, maar door de groeiende vraag is het verstandig om niet te lang te wachten met je aanvraag.
products:
- name: Vaillant aroTHERM plus
  url: https://www.vaillant.nl/consumenten/producten/warmtepompen/arotherm-plus/
  price: '8000'
- name: Daikin Altherma 3
  url: https://www.daikin.nl/nl_nl/producten.html
  price: '9000'
schema_type: Article
---
Voor een hybride warmtepomp bedraagt de ISDE-subsidie in 2026 doorgaans tussen de €2.500 en €3.500, afhankelijk van het type en het vermogen — vaak bijna de helft van de netto investering. Het aanvraagproces loopt via mijn.rvo.nl en de aanvraag zelf is in een half uur gedaan; op de uitbetaling wacht je enkele maanden. In deze gids staat stap voor stap welke bedragen je kunt verwachten, hoe je de aanvraag doet en hoe je de ISDE combineert met gemeentelijke subsidies.

*Disclosure: de links naar ATAG in dit artikel zijn gewone verwijzingen — wij hebben met deze partij geen affiliate- of commissierelatie.*


💡 *Niet zeker over de saldering-stop in 2027? Lees de [Saldering 2027 transitie-planner](/posts/saldering-2027-transitie-planner/) — 5 strategieën om €500-€2000/jaar veilig te stellen.*

> **Kort antwoord:** Alles over de ISDE subsidie in 2026: bedragen voor warmtepompen en isolatie, hoe aanvragen, voorwaarden en combinatie met andere subsidies.
>
> De ISDE subsidie voor een warmtepomp varieert van €3.000 tot €5.800 afhankelijk van het type. Een hybride warmtepomp levert circa €3.000 op, een lucht-water warmtepomp €3.000-€3.500, en een bodem-water warmtepomp €4.500-€5.800. Het exacte bedrag hangt af van het vermogen en de SCOP-waarde.

## Wat is de ISDE?

De ISDE is een rijkssubsidie van de Rijksdienst voor Ondernemend Nederland (RVO) die eigenaar-bewoners stimuleert om hun woning te verduurzamen. De subsidie is beschikbaar voor twee categorieën maatregelen:

1. **Warmtepompen** — Hybride, lucht-water en bodem-water warmtepompen
2. **Isolatie** — Gevelisolatie, dakisolatie, vloerisolatie, spouwmuurisolatie en HR++ glas

De ISDE geldt alleen voor **bestaande woningen** (minimaal 2 jaar oud) en is bedoeld voor **eigenaar-bewoners**.

## Subsidiebedragen 2026

### Warmtepompen

| Type warmtepomp | Subsidiebedrag | Typische investering | Netto kosten na subsidie |
|----------------|---------------|---------------------|------------------------|
| **Hybride warmtepomp** | ~€3.000 | €4.000-€7.000 | €1.000-€4.000 |
| **Lucht-water warmtepomp** | €3.000-€3.500 | €8.000-€14.000 | €5.000-€10.500 |
| **Bodem-water warmtepomp** | €4.500-€5.800 | €15.000-€25.000 | €10.500-€19.200 |
| **Lucht-lucht warmtepomp** | Niet subsidiabel | €2.000-€5.000 | Niet subsidiabel |

**Let op:** Het exacte subsidiebedrag voor warmtepompen hangt af van het vermogen (kW) en de SCOP-waarde (seizoensgebonden efficiëntie). De RVO publiceert een apparatenlijst met alle goedgekeurde warmtepompen en bijbehorende bedragen.

### Isolatie

| Isolatiemaatregel | Subsidiebedrag per m² | Typisch bedrag per woning |
|-------------------|----------------------|--------------------------|
| **Spouwmuurisolatie** | €5-€9/m² | €700-€1.500 |
| **Gevelisolatie (binnenzijde)** | €18-€25/m² | €1.500-€3.000 |
| **Gevelisolatie (buitenzijde)** | €25-€35/m² | €2.500-€5.000 |
| **Dakisolatie** | €15-€25/m² | €1.500-€3.300 |
| **Vloerisolatie** | €5-€10/m² | €500-€1.200 |
| **HR++ glas** | €35-€60/m² | €2.000-€5.000 |
| **Triple glas** | €50-€80/m² | €3.000-€7.000 |

**Belangrijk:** Voor isolatie geldt een minimum oppervlakte per maatregel. Controleer de actuele eisen op rvo.nl.

## Stap-voor-stap: ISDE subsidie aanvragen

### Stap 1: Controleer of je in aanmerking komt

Voorwaarden:
- Je bent eigenaar-bewoner van een bestaande woning (minimaal 2 jaar oud)
- De maatregel wordt uitgevoerd door een gecertificeerd installateur
- Het product staat op de RVO-apparatenlijst (voor warmtepompen)
- De isolatie voldoet aan de minimum Rd-waarde eisen

### Stap 2: Kies een gecertificeerd installateur

- Vraag minimaal 3 offertes aan
- Controleer of de installateur gecertificeerd is (bijv. STEK voor warmtepompen)
- Vraag expliciet of het gekozen product op de RVO-apparatenlijst staat
- Laat de installatie uitvoeren en bewaar alle documenten

### Stap 3: Verzamel de benodigde documenten

Na de installatie heb je nodig:
- **Factuur** van de installateur (met specificatie van product, type en hoeveelheid)
- **Bewijs van betaling** (bankafschrift met overboeking)
- **Productspecificaties** (merk, type, serienummer)
- **Foto's** van de uitgevoerde werkzaamheden (voor isolatie)
- **DigiD of eHerkenning** voor het inloggen op mijn.rvo.nl

### Stap 4: Dien de aanvraag in

1. Ga naar **mijn.rvo.nl**
2. Log in met DigiD
3. Kies "ISDE" bij de subsidieaanvragen
4. Vul het aanvraagformulier in
5. Upload de benodigde documenten
6. Controleer en verstuur

### Stap 5: Wacht op beoordeling en uitbetaling

- RVO beoordeelt je aanvraag binnen 8-13 weken
- Bij goedkeuring wordt het subsidiebedrag direct overgemaakt
- Bij ontbrekende documenten krijg je een verzoek tot aanvulling

**Deadline:** Je moet de aanvraag indienen binnen **12 maanden** na de datum op de factuur.

## Combinatie met andere subsidies

De kracht van de ISDE zit in de combinatiemogelijkheden. Door slim te combineren maximaliseer je je financiële voordeel.

### ISDE + Gemeentelijke subsidie

Veel gemeenten bieden aanvullende subsidies bovenop de ISDE:
- Warmtepompsubsidie: €500-€2.500 extra
- Isolatiesubsidie: €500-€1.500 extra
- Verduurzamingsleningen: 0% rente

**Tip:** Check de website van je gemeente of energieloket voor lokale regelingen. Deze variëren sterk per gemeente.

### Twee of meer maatregelen: hogere ISDE-bedragen

Er is geen aparte isolatiesubsidie meer bovenop de ISDE. De oude SEEH (Subsidie Energiebesparing Eigen Huis) kon voor het laatst tot en met 31 december 2022 worden aangevraagd; voor eigenaar-bewoners is de isolatiesubsidie in de ISDE ondergebracht en voor VvE's is per 23 januari 2023 de SVVE in de plaats gekomen.

Wat er wél is: binnen de ISDE zelf gelden hogere subsidiebedragen per isolatiemaatregel wanneer je twee of meer maatregelen uitvoert. Voer je één maatregel uit, dan krijg je het lagere bedrag per vierkante meter. De actuele bedragen per maatregel — en de vraag of jouw combinatie kwalificeert — staan op rvo.nl/isde. Reken dus niet met een vast bonuspercentage, maar zoek de bedragen op voor de maatregelen die jij laat uitvoeren.

### ISDE + Nationaal Warmtefonds

Het Nationaal Warmtefonds biedt leningen voor verduurzaming tegen zeer lage rente (soms 0%). Dit is geen subsidie maar kan gecombineerd worden met de ISDE:
- Maximaal leenbedrag: €65.000
- Rente: 0-3,5% afhankelijk van inkomen
- Looptijd: tot 20 jaar
- Geen boeterente bij vervroegde aflossing

### ISDE + Energiebelasting teruggave

Als je een warmtepomp installeert en hierdoor minder gas verbruikt, bespaar je ook op energiebelasting. Dit is geen subsidie maar een extra financieel voordeel dat meeweegt in de terugverdientijd.

## Tips voor maximale subsidie

1. **Combineer maatregelen** — De ISDE keert per isolatiemaatregel een hoger bedrag uit als je er twee of meer laat uitvoeren
2. **Check je gemeente eerst** — Sommige gemeenten eisen dat je hun subsidie aanvraagt vóór de installatie
3. **Gebruik de RVO-apparatenlijst** — Controleer altijd of je warmtepomp op de lijst staat vóór aankoop
4. **Bewaar alles** — Facturen, betaalbewijzen, foto's en specificaties. Ontbrekende documenten vertragen je aanvraag
5. **Vraag snel aan** — Het ISDE-budget is eindig. Bij overschrijding worden aanvragen op volgorde van binnenkomst afgehandeld
6. **Laat je adviseren** — Een energieadvies (€300-€500) kan uitwijzen welke combinatie het meest oplevert

## Hoe werkt de RVO-apparatenlijst precies?

De ISDE-subsidie voor warmtepompen is niet voor elk product beschikbaar — alleen voor warmtepompen die op de officiële RVO-erkende apparatenlijst staan. Dit is een praktisch punt dat veel fouten veroorzaakt.

**Hoe de lijst werkt:**
- Fabrikanten laten hun producten testen door een erkend testlaboratorium
- RVO controleert de testresultaten en neemt het product op bij goedkeuring
- Per product (inclusief exact typenummer, variant en vermogen) staat een specifiek subsidiebedrag
- De lijst wordt meerdere keren per jaar bijgewerkt

**Praktisch:**
- Vraag altijd je installateur om het volledige typenummer (inclusief suffix, bijv. "Daikin EBLA06D3W1" in plaats van "Daikin Altherma 3")
- Controleer dit typenummer zelf op rvo.nl/subsidies-financiering/isde via de zoekfunctie
- Als het type niet op de lijst staat: vraag de installateur om een alternatief model of om de fabrikant te vragen het product aan te melden

Dit gaat mis wanneer een installateur een type levert dat net van de RVO-apparatenlijst is afgevoerd, bijvoorbeeld na een productupdate van de fabrikant. De aanvraag moet dan worden ingetrokken en je hebt een correctiefactuur met het nieuwe typenummer nodig — reken op weken vertraging. Voorkom dit door het typenummer op je offerte zelf te controleren op rvo.nl vóór de installatie.

## Terugverdientijd met ISDE subsidie

De ISDE maakt verduurzaming aanzienlijk aantrekkelijker. Enkele voorbeelden:

| Maatregel | Investering | ISDE subsidie | Netto investering | Jaarlijkse besparing | Terugverdientijd |
|-----------|-----------|--------------|-------------------|---------------------|-----------------|
| Hybride warmtepomp | €5.500 | €3.000 | €2.500 | €600-€900 | 3-4 jaar |
| Spouwmuurisolatie | €1.200 | €800 | €400 | €200-€350 | 1-2 jaar |
| Dakisolatie | €3.000 | €1.800 | €1.200 | €350-€500 | 2-3 jaar |
| HR++ glas | €4.000 | €2.200 | €1.800 | €250-€400 | 4-7 jaar |

Met subsidie worden de terugverdientijden zo kort dat vrijwel elke verduurzamingsmaatregel financieel aantrekkelijk is.

## Veelgemaakte fouten

- **Te laat aanvragen** — De deadline is 12 maanden na factuurdatum, maar eerder is beter
- **Verkeerd product** — Niet alle warmtepompen staan op de RVO-lijst. Controleer vóór aankoop
- **Ontbrekende documenten** — Incomplete aanvragen leiden tot weken vertraging
- **Geen gecertificeerd installateur** — De installatie moet door een erkend bedrijf worden uitgevoerd
- **Nieuwbouw** — De ISDE geldt niet voor nieuwbouwwoningen

## Specifieke ISDE-bedragen per warmtepompmerk 2026

De RVO publiceert de exacte subsidiebedragen per product op de apparatenlijst. Hieronder de indicatieve bedragen voor de meest populaire modellen in Nederland:

### Hybride warmtepompen

| Model | Vermogen | ISDE 2026 (indicatief) |
|-------|---------|----------------------|
| Remeha Elga Ace HT | 4 kW | €2.700 |
| Remeha Elga Ace HT | 6 kW | €2.900 |
| Remeha Elga Ace HT | 8 kW | €3.200 |
| Daikin Altherma 3 H Hybrid | 4 kW | €2.600 |
| Daikin Altherma 3 H Hybrid | 6 kW | €2.900 |
| ATAG ENERGION hybride | 6 kW | €2.700 |
| ATAG ENERGION hybride | 8 kW | €3.000 |
| Bosch Compress 7800i hybride | 6 kW | €2.800 |
| Bosch Compress 7800i hybride | 8 kW | €3.200 |
| Vaillant aroTHERM plus hybride | 5 kW | €2.700 |

### Lucht-water warmtepompen (all-electric)

| Model | Vermogen | ISDE 2026 (indicatief) |
|-------|---------|----------------------|
| Daikin Altherma 3 H/M | 6 kW | €3.100 |
| Daikin Altherma 3 H/M | 8 kW | €3.700 |
| Daikin Altherma 3 H/M | 11 kW | €4.200 |
| Mitsubishi Ecodan | 8 kW | €3.500 |
| Vaillant aroTHERM plus | 8 kW | €3.400 |
| ATAG ENERGION all-electric | 12 kW | €3.800 |

**Let op:** dit zijn indicatieve bedragen op basis van de RVO-lijst van begin 2026. Het exacte bedrag per typenummer staat op rvo.nl. Kleine subvarianten (HT vs. standaard, R32 vs. R290) kunnen verschillen.

## De aanvraag stap voor stap — en waar het blijft hangen

**Direct na installatie:** de installateur stelt de warmtepomp in dienst en levert de factuur. Controleer het typenummer op de factuur nog één keer tegen de RVO-apparatenlijst; wijkt het af van de offerte, meld dat direct.

**Documenten verzamelen.** Je hebt nodig:
- de factuur met typenummer en serienummer;
- een betaalbewijs (bankafschrift of screenshot van de betaling);
- een foto van de buitenunit waarop het serienummerlabel leesbaar is;
- een bewijs van eigendom van de woning.

Dat laatste punt is waar de meeste aanvragen vertraging oplopen. Je naam op de factuur is niet voldoende: RVO vraagt een uittreksel uit het kadaster of een kopie van de koopakte. Haal dat kadastraal uittreksel daarom vóóraf op — dat kost rond de €11,50 via kadaster.nl en het scheelt je dagen wachten halverwege de aanvraag.

**De aanvraag zelf** doe je op mijn.rvo.nl met DigiD (of eHerkenning). Het formulier is in een half uur gedaan als je alle documenten klaar hebt liggen. Je ontvangt direct een bevestigingsmail.

**Daarna wachten.** Op de goedkeuringsbeschikking wacht je in de praktijk enkele maanden; RVO betaalt binnen een paar werkdagen na de beschikking uit. Reageer direct als RVO om aanvullende informatie vraagt — elke dag vertraging in je antwoord schuift de uitbetaling op.

### Veelgestelde praktijkvragen

**"Mijn installateur zegt dat hij de ISDE voor mij aanvraagt. Mag dat?"** (deze en de volgende vragen komen het vaakst terug)

Ja, dat kan. De installateur kan namens jou een machtiging aanvragen en de ISDE indienen. De uitbetaling gaat dan ofwel naar jou, ofwel — als jullie dat afspraken — wordt het bedrag verrekend in de factuur. Leg dit schriftelijk vast in de opdracht.

**"Ik heb de warmtepomp al drie maanden geleden laten installeren en nog geen ISDE aangevraagd. Kan dat nog?"**

Ja. De deadline is 12 maanden na de factuurdatum. Maar wacht niet te lang: ontbrekende documenten of een uitgeput budget kunnen je verrassen. Ons advies is om binnen drie maanden aan te vragen.

**"De factuur staat op naam van mijn partner maar het huis is van mij. Probleem?"**

Dat kan een probleem zijn. De ISDE-aanvraag moet op naam van de eigenaar-bewoner staan. Zorg dat de factuur op naam staat van de persoon die eigenaar-bewoner is (staat in het kadaster). Als dit niet klopt, is naherstel mogelijk maar tijdrovend.

**"Ik heb via Quatt een warmtepomp als abonnement. Kan ik ISDE aanvragen?"**

Nee. Bij een abonnement is Quatt de eigenaar van de warmtepomp, niet jij. Eigendom is een vereiste voor de ISDE. Dit is een concreet financieel nadeel van het abonnementsmodel.

## Budgetplafond ISDE 2026: wanneer zijn aanvragen op?

Het totale ISDE-budget voor 2026 bedraagt circa €328 miljoen. In de jaren 2022 en 2023 was het budget vroeg in het jaar uitgeput, waarna aanvragen werden aangehouden. In 2024 en 2025 was het budget ruimer, mede door extra middelen vanuit het klimaatakkoord.

Voor 2026 gaat RVO ervan uit dat het budget het hele jaar toereikend is, maar garanties zijn er niet. Ons advies: dien je aanvraag in zodra je alle documenten hebt en wacht niet tot het einde van het jaar.

Een indicatie: in 2025 werden circa 95.000 warmtepompen geïnstalleerd in Nederland. Bij een gemiddelde subsidie van €3.000 is dat al €285 miljoen aan warmtepompsubsidie alleen. Het budget is krap.

## Combinatie van maatregelen: zo maximaliseer je subsidie in 2026

De slimste aanpak is niet één maatregel maar een pakket. Hier zijn drie realistische combinaties:

### Pakket 1: hybride warmtepomp + spouwmuurisolatie

Voor een gemiddeld rijtjeshuis (1975-1990, label C/D):

| Maatregel | Investering | ISDE | Netto |
|-----------|-------------|------|-------|
| Hybride warmtepomp (6 kW) | €6.800 | €2.900 | €3.900 |
| Spouwmuurisolatie (45 m²) | €1.350 | €810 | €540 |
| **Totaal** | **€8.150** | **€3.710** | **€4.440** |

Jaarlijkse besparing (gecombineerd effect): €900-€1.100. Terugverdientijd: 4-5 jaar.

### Pakket 2: full-electric warmtepomp + dakisolatie + HR++ glas

Voor een woning die al spouwmuurisolatie heeft (label C) maar nog niet optimaal:

| Maatregel | Investering | ISDE | Netto |
|-----------|-------------|------|-------|
| Full-electric warmtepomp (8 kW) | €11.500 | €3.700 | €7.800 |
| Dakisolatie (80 m²) | €3.200 | €2.000 | €1.200 |
| HR++ glas (20 m²) | €3.800 | €1.400 | €2.400 |
| **Totaal** | **€18.500** | **€7.100** | **€11.400** |

Jaarlijkse besparing: €1.400-€1.800. Terugverdientijd: 6-8 jaar.

### Pakket 3: bodemsonde + complete isolatie

Voor wie serieus all-in wil gaan:

| Maatregel | Investering | ISDE | Netto |
|-----------|-------------|------|-------|
| Bodem-water warmtepomp | €20.000 | €5.200 | €14.800 |
| Complete isolatiepakket | €8.000 | €4.500 | €3.500 |
| **Totaal** | **€28.000** | **€9.700** | **€18.300** |

Jaarlijkse besparing: €2.200-€2.800. Terugverdientijd: 7-8 jaar. SCOP van een bodemsonde-pomp ligt gemiddeld 1 punt hoger dan lucht-water — dat telt op.

## Nationaal Warmtefonds: lenen voor verduurzaming

Niet iedereen heeft €3.000-€15.000 vrij liggen voor een warmtepomp. Het Nationaal Warmtefonds biedt een oplossing: een gebouwgebonden lening die bij de woning hoort, niet bij de persoon.

**Kenmerken Nationaal Warmtefonds 2026:**
- Maximaal €65.000 per woning
- Rente: 0,0% bij laag inkomen (tot 130% van bijstandsnorm), anders 2-3,5%
- Looptijd: maximaal 20 jaar
- Geen boeterente bij vervroegde aflossing
- De lening gaat mee met de woning bij verkoop (koper neemt over, of jij lost af)

Bij een lening van €5.000 tegen 2,5% over 10 jaar betaal je circa €47/maand. Als de warmtepomp je €75/maand bespaart, heb je netto direct winst.

## Verlengde FAQ: praktische vragen over ISDE

**Kun je ISDE aanvragen als de factuur al betaald is maar de installateur de aanvraag vergeten is?**

Ja, zolang je maar binnen 12 maanden na factuurdatum indient. Je kunt de aanvraag zelf doen via mijn.rvo.nl — je hebt geen installateur nodig voor het indienen. Je hebt wel nodig: de factuur, het betaalbewijs, de productspecificaties (typenummer, serienummer) en foto's van de installatie. De installateur heeft zijn taak gedaan door te installeren; de aanvraag is jouw verantwoordelijkheid (tenzij contractueel anders afgesproken).

**Twee woningen op één kadastraal nummer: is voor beide ISDE aan te vragen?**

Nee. De ISDE is per adres/wooneenheid maximaal één keer te benutten voor hetzelfde type maatregel. Bij een tweekapper met twee afzonderlijke adressen en kadastrale nummers: beide woningen komen elk afzonderlijk in aanmerking. Bij een gecombineerd kadastraal perceel: vraag RVO direct om uitsluitsel.

**Installatie in december 2025, aanvraag in 2026: gelden de 2025- of de 2026-tarieven?**

De ISDE-tarieven die gelden zijn die van het moment van installatie (factuurdatum). Als je factuur december 2025 is, gelden de 2025-tarieven — ook als je pas in 2026 indient. Het budget waaruit betaald wordt, is het budget van het aanvraagjaar (2026). De tarieven zijn historisch gezien stabiel gebleven, dus het verschil is doorgaans nihil.

**Kun je ISDE laten aanvragen via een subsidiebureau?**

Ja, er zijn adviesbureaus die de ISDE voor een vaste vergoeding voor je aanvragen (€75-€200). Dit kan handig zijn als je moeite hebt met DigiD of de RVO-formulieren. Let op: RVO is gratis, een bemiddelaar kost geld. Voor de meeste mensen is het zelf doen de verstandigste keuze.

**Wat als de warmtepomp na twee jaar kapot gaat — moet de subsidie dan terug?**

Nee. De ISDE-subsidie wordt uitbetaald na installatie en is niet gebonden aan een verplichte gebruiksduur. Als de warmtepomp kapot gaat, is de subsidie al ontvangen en hoef je die niet terug te betalen. Wel verlies je het rendement van de investering — maar dat is een apart risico.

**Een warmtepomp huren (bij Quatt of vergelijkbaar): is daar ISDE op mogelijk?**

Nee. Bij huur of lease-constructies is de installateur (het bedrijf) eigenaar van de warmtepomp, niet jij. ISDE vereist eigendom door de eigenaar-bewoner. Dit is een expliciet nadeel van abonnementsmodellen.

## Hoe lang duurt de ISDE-uitbetaling? Een realistische tijdlijn

Een van de meest gestelde vragen: wanneer staat het geld op je rekening? Hieronder een realistische tijdlijn. RVO noemt zelf een beslistermijn; de doorlooptijd in de praktijk hangt sterk af van hoe compleet je aanvraag is en van de drukte bij RVO.

**Dag 0:** Installatie voltooid, factuur ontvangen.

**Dag 7-14:** Alle documenten verzameld (factuur, betaalbewijs, foto's, productspecificaties, kadasteruittreksel).

**Dag 14-21:** Aanvraag ingediend via mijn.rvo.nl. Je ontvangt direct een bevestigingsmail.

**Week 4-6:** RVO stuurt soms een verzoek tot aanvulling als er iets ontbreekt. Reageer hier snel op — elke dag vertraging schuift de uitbetaling op.

**Week 8-13:** Goedkeuringsbeschikking ontvangen. RVO betaalt binnen 3-5 werkdagen na de beschikking.

**Week 13-16 (drukke perioden):** In het najaar (september-november) lopen de behandeltijden op vanwege het hoge volume aanvragen. Plan hier rekening mee.

**Kortom:** reken bij een complete aanvraag op ongeveer twee tot drie maanden tussen installatie en uitbetaling. Zorg dat je financiering die periode overbrugt — de installateur wil doorgaans binnen dertig dagen betaald worden, dus je schiet de subsidie tijdelijk voor.

## De ISDE in perspectief: is het nog steeds genoeg?

Een terugkerend bezwaar: "De subsidie is €3.000, maar de warmtepomp kost €8.000. Dat gat is te groot."

Dat is een reëel punt. Maar het perspectief hoort erbij. In 2010 was er geen ISDE. In 2015 was het budget €15 miljoen. In 2026 is het €328 miljoen. De subsidie is de afgelopen 10 jaar vertienvoudigd. En de kosten van warmtepompen zijn in dezelfde periode gedaald — een hybride van €6.000 (exclusief installatie) kostte in 2018 nog €9.000.

De trend wijst op verdere dalingen in aanschafprijs naarmate de productie schaalvoordelen pakt. Tegelijk groeit de druk op het ISDE-budget naarmate meer huishoudens verduurzamen. Op welk moment je precies het beste kunt instappen, weet niemand. Maar wachten puur op een betere subsidie is zelden de slimste strategie.

## Lees ook

- **[Zonnepanelen Subsidie Nederland 2026](/posts/zonnepanelen-subsidie-nederland-2026/)** — Alle subsidies voor zonnepanelen op een rij
- **[Thuisbatterij Terugverdientijd Berekenen](/posts/thuisbatterij-terugverdientijd-berekenen-2026/)** — Bereken of een thuisbatterij rendabel is
- **[Beste Thuisbatterij Nederland 2026](/posts/beste-thuisbatterij-nederland-2026/)** — De top 5 thuisbatterijen vergeleken
- **[Afbouw Salderingsregeling 2026](/posts/salderingsregeling-afbouw-wat-betekent-het-2026/)** — Wat betekent het voor jou?

---

*Laatst bijgewerkt: april 2026.*


<a href="https://go.duurzaamthuislab.nl/atag" class="cta-affiliate" rel="nofollow noopener sponsored" target="_blank">Bekijk Atag</a>


<a href="https://go.duurzaamthuislab.nl/atag" class="cta-affiliate" rel="nofollow noopener sponsored" target="_blank">Bekijk Atag</a>

---

**Externe bron:** [RVO — ISDE-subsidie info](https://www.rvo.nl/subsidies-financiering/isde) — onafhankelijke informatie over dit onderwerp.
